...
public:
    ...
    void LoadTraj_fromVector(const std::pair<std::vector<double>, std::vector<Sophus::SE3f>>& IESEKFposes, const std::pair<std::vector<double>, std::vector<Sophus::SE3f>>& vslamtimeposes);
...
template<typename Type>
Eigen::Matrix<Type, 4, 4> ComputeSim3<Type>::GetSE3() {
    ...
    Eigen::Matrix<Type, 4, 4> se3 = Eigen::Matrix<Type, 4, 4>::Identity();

    // Compute the rotation matrix
    se3.block(0, 0, 3, 3).noalias() = svd.matrixU() * S.asDiagonal() * svd.matrixV().transpose();

    // Compute the translation vector
    se3.block(0, 3, 3, 1) = means_2 - se3.block(0, 0, 3, 3) * means_1;
...
template<typename Type>
void ComputeSim3<Type>::LoadTraj_fromVector(const std::pair<std::vector<double>, std::vector<Sophus::SE3f>>& IESEKFposes, const std::pair<std::vector<double>, std::vector<Sophus::SE3f>>& vslamtimeposes) {
    _vTraj_1.clear();
    _vTraj_2.clear();

    // Load IESEKFposes
    for(size_t i = 0; i < IESEKFposes.first.size(); i++) {
        TrajPoint<Type> trajPoint;
        trajPoint.timestamp = static_cast<Type>(IESEKFposes.first[i]);
        trajPoint.translation = IESEKFposes.second[i].translation().template cast<Type>();
        
        Eigen::Quaternionf quat = IESEKFposes.second[i].unit_quaternion();
        trajPoint.rotation << static_cast<Type>(quat.w()),
                              static_cast<Type>(quat.x()),
                              static_cast<Type>(quat.y()),
                              static_cast<Type>(quat.z());

        _vTraj_1.emplace_back(trajPoint);
    }

    // Load vslamtimeposes
    for(size_t i = 0; i < vslamtimeposes.first.size(); i++) {
        TrajPoint<Type> trajPoint;
        trajPoint.timestamp = static_cast<Type>(vslamtimeposes.first[i]);
        trajPoint.translation = vslamtimeposes.second[i].translation().template cast<Type>();
        
        Eigen::Quaternionf quat = vslamtimeposes.second[i].unit_quaternion();
        trajPoint.rotation << static_cast<Type>(quat.w()),
                              static_cast<Type>(quat.x()),
                              static_cast<Type>(quat.y()),
                              static_cast<Type>(quat.z());
