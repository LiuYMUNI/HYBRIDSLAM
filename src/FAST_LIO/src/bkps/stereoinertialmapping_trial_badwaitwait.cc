  ...
  while(status)
  {
    ...
    
    {
        ...
        if(!frame_time_queue.empty() && !lidar_time_queue.empty())
        {
            ...
            {
                std::cout << "frame_time < lidar_time " << std::endl;
    ...
    {
        ros::spinOnce();
        // std::cout << "waiting for timestamp to be processed" << frame_time_str << std::endl; 
        continue;
    }
    data_ready = true;
            ...
            if(frame_time >= lidar_time_front)
            {
                if(bfirst)
                {
                    ...
                    while(frame_time >= lidar_time_front)
                    {
                        ...
                        if(lidar_time_queue_for_vslam.size()>=2)
                        {
                            std::cout << "lidar_time_queue_for_vslam.size()>=2" << std::endl;
                            {
                                std::lock_guard<std::mutex> lock(lidar_time_queue_for_vslam_mutex);
                    ...
                    int trackingstate = mpSLAM->mtrackingStat;
                    if(trackingstate != 2)
                    {
                        std::cout << "trackingstate != 2, VSLAM reseted" << std::endl;
                        std::cout << "trackingstate = " << trackingstate << std::endl;
                        nTrackingstate = 0;
                    
                    }
                    else if(trackingstate == 2)
                    {
                        std::cout << "trackingstate == 2, VSLAM tracking" << std::endl;
                        std::cout << "trackingstate = " << trackingstate << std::endl;
                        nTrackingstate += 2;
                    }
                    if(bVIBA0 && !bVIBA2)
                    ...
                    // if(nTrackingstate <= 6)
                    // {
                    //     std::cout << "nTrackingstate <= 6" << std::endl;
                    //     bVIBA0 = mpSLAM->mbInertialBA0;
                    //     DataEntry Measures;
                    ...
                    if(!bVIBA0)
                    {
                        std::cout << "nTrackingstate > 6 && !bVIBA0" << std::endl;
                        bVIBA0 = mpSLAM->mbInertialBA0;
                        Twc = Tcw_.inverse();
                        Twi = Twc*Tleftb;
                        Tiwi = Tbleft*Twi;
                    ...
                    {
                        std::cout << "nTrackingstate > 6 && bVIBA0 && !bVIBA1" << std::endl;
                    ...
                    {
                        std::cout << "nTrackingstate > 6 && bVIBA0 && bVIBA1 && !bVIBA2" << std::endl;
                    ...
                    {
                        std::cout << "nTrackingstate > 6 && bVIBA2" << std::endl;
                        Eigen::Matrix3f Ryc0 = mpSLAM->mRwg;
                        Eigen::Matrix3d Ryc0_ = Ryc0.cast<double>();
                        Eigen::JacobiSVD<Eigen::Matrix3d> svd_wg(Ryc0_, Eigen::ComputeFullU | Eigen::ComputeFullV);
                        Eigen::Matrix3d Ryc0_orthogonal = svd_wg.matrixU() * svd_wg.matrixV().transpose();
                ...
                else
                {
                    ...
                    // convert the Tcw to a SE3d type
                    Sophus::SE3d Tcw_(Rcw_orthogonal, tcw);
                    std::cout << "nTrackingstate = " << nTrackingstate << std::endl;
                    bVIBA0 = mpSLAM->mbInertialBA0;
                    // if(nTrackingstate <= 6)
                    // {
                    //     std::cout << "nTrackingstate <= 6" << std::endl;
                    ...
                    //     dataStore.addData(frame_time, Measures);
                    //     std::string frame_time_str = std::to_string(frame_time);
                    //     std::cout << "frame_time_str added = " << frame_time_str << std::endl;
                    // }
                    // else 
                    if(!bVIBA0)
                    {
