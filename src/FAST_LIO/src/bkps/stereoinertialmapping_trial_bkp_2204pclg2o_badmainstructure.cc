        ...
        if (!imgLeftBuf.empty()&&!imgRightBuf.empty()&&!mpImuGb->imuBuf.empty())
        {
            ...
            if(lidar_time_queue_for_vslam.size() > 1)
            {
                ...
                if(frame_time < last_lidar_time)
                {
                    ...
                    last_processed_lidartime = second_lidar_time;
                }
                else if(last_lidar_time <= frame_time && frame_time <= second_lidar_time)
                {
                    ...
                    while(!lidarPRO.hasData(last_lidar_time))
                    {
                        std::string last_lidar_time_str = std::to_string(last_lidar_time);
                        std::cout << "waiting for the lidar to be processed" << last_lidar_time_str << std::endl;
                    ...
                    {
                        LidarProcess lidarprocess = lidarPRO.getData(last_lidar_time);
                        Measures_.lidar = lidarprocess.feats_down_body_fororb;
                ...
                {
                    double time_diff = frame_time - second_lidar_time;
                    std::cout << "time difference = " << time_diff << std::endl;
                    double delta_t = 1.0/15.0;
                    if(time_diff > delta_t)
                    {
                        std::cout << "time difference is greater than delta_t" << std::endl;
                    }
                    else 
                    {
                        std::cout << "time difference is less than delta_t" << std::endl;
                    }
