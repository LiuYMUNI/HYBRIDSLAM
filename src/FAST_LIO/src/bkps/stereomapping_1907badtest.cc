...
int main(int argc, char **argv)
{
  ...
  // Create SLAM system. It initializes all system threads and gets ready to process frames.
    ...
    message_filters::Subscriber<sensor_msgs::Image> left_sub(no, "/uas_cam2/image", 200000);
    message_filters::Subscriber<sensor_msgs::Image> right_sub(no, "/uas_cam4/image", 200000);
    ...
    while(status)
    {
        ...
        
        {
            ...
            {
                if(frame_time_queue.empty())
                {
                    std::cout << "frame_time_queue is empty" << std::endl;
                }
                if(lidar_time_queue.empty())
                {
                    std::cout << "lidar_time_queue is empty" << std::endl;
                }
                continue;
            }
        }
        ...
        if(globalMeasures.frame_or_not)
        {
            ...
            if(bfirst_0)
            {
                std::cout << "bfirst_0 = true" << std::endl;
                bfirst_0 = false;
                continue;
            }
            else
            {
                std::cout << "bfirst_0 = false" << std::endl;
    ...
    else
    {
        ...
        if(frame_time >= lidar_time_front)
        {
            ...
            else
            {
                ...
                // convert the frame time to string
                std::string frame_time_str = std::to_string(frame_time);
                // get the lidar time front
                std::string lidar_time_front_str = std::to_string(lidar_time_front);
                std::cout << "frame_time_str = " << frame_time_str << std::endl;
                std::cout << "lidar_time_front_str = " << lidar_time_front_str << std::endl;
                while(frame_time >= lidar_time_front)
                {
                    while(!lidarPRO.hasData(lidar_time_front))
                    {
                ...
                if(!bVIBA0)
                {
                    ...
                    if(globalcounter > 0)
                    {
                        ...
                        if(true)
                        {
                            ...
                            Tcw = mpSLAM->TrackStereo_Pose(cv_ptrLeft->image,cv_ptrRight->image,cv_ptrLeft->header.stamp.toSec(),vImuMeas, "", Tc0bn, information_matrix, velocity_c0.cast<float>());
                            double t_track_end = omp_get_wtime();
                            Consumed_time consumed_time_entry;
                            consumed_time_entry.timestamp = ros::Time().fromSec(frame_time);
                            consumed_time_entry.time = t_track_end - t_track_start;
                            ...
                            if(!bLoopDetected)
                            {
                                if(nLoopBuffer > 0)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        Tcw = mpSLAM->TrackStereo_Pose(cv_ptrLeft->image,cv_ptrRight->image,cv_ptrLeft->header.stamp.toSec(),vImuMeas, "", Tc0bn, information_matrix);
                                    }
                                    // Tcw = mpSLAM->TrackStereo(cv_ptrLeft->image,cv_ptrRight->image,cv_ptrLeft->header.stamp.toSec());
                                }
                                else
