...
bool sync_packages_stereo_noDe(MeasureGroup &meas, double frame_time_)
{
    deque<sensor_msgs::Imu::ConstPtr> imu_buffer_noDe;
    // copy the imu buffer using the mutex
    {
        std::lock_guard<std::mutex> lock(mtx_buffer);
        imu_buffer_noDe = imu_buffer;
    }

    if (imu_buffer_noDe.empty()) {
        ////std::cout << "IMU buffer empty" << std::endl;
        return false;
    }
    /*** push a lidar scan ***/
    ...
    /*** push imu data, and pop from imu buffer ***/
    double imu_time = imu_buffer_noDe.front()->header.stamp.toSec();
    std::string imu_time_str = std::to_string(imu_time);
    ////std::cout << "IMU time front: " << imu_time_str << std::endl;
    ////std::cout << "IMU time 1 " << std::endl;
    if(imu_time > frame_time_){
    ...
    meas.imu.clear();
    while ((!imu_buffer_noDe.empty()) && (imu_time < frame_time_))
    {
        // ////std::cout << "IMU time: " << imu_time << std::endl;
        imu_time = imu_buffer_noDe.front()->header.stamp.toSec();
        if(imu_time > frame_time_) break;
        meas.imu.push_back(imu_buffer_noDe.front());
        imu_buffer_noDe.pop_front();
    }
    ////std::cout << "IMU time 2 " << std::endl;
    return true;
}
        ...
        if (!imgLeftBuf.empty()&&!imgRightBuf.empty()&&!mpImuGb->imuBuf.empty())
        {
            ...
            if(frame_time >= lidar_time_front)
            {
                ...
                else
                {
                    ...
                    {
                        std::cout << "lidar degenerate" << std::endl;
                        bVIBA0 = mpSLAM->mbInertialBA0;
                        if(bVIBA0)
                        {
                    ...
                    {
                        std::cout << "no degenerate" << std::endl;
                        bVIBA0 = mpSLAM->mbInertialBA0;
                        if(bVIBA0)
                        {
                            ////std::cout << "now the frame time is smaller than lidar time front "<< std::endl;
                            PointCloudXYZI::Ptr feats_down_body_fororbg(new PointCloudXYZI());
                            esekfom::esekf<state_ikfom, 12, input_ikfom> kf_noDe;
                            double epsi[23] = {0.001};
                            fill(epsi, epsi+23, 0.001);
                            kf_noDe.init_dyn_share(get_f, df_dx, df_dw, h_share_model_stereo_prior, NUM_MAX_ITERATIONS, epsi);
                            if(sync_packages_stereo_noDe(Measures_, frame_time))
                            {
                                Measures_.lidar = globalLidarProcess.feats_down_body_fororb;
                                ...
                                if(!bfirstlocal)
                                {
                                    ...
                                    state_point = kf.get_x();
                                    kf_noDe.change_x(state_point);
                                    auto P_ = kf.get_P();
                                    kf_noDe.change_P(P_);
                                    // process imu here because of PCL undistortion
                                    p_imu->Process_stereo(Measures_, kf_noDe, feats_down_body_fororbg);
                                }
                            }
                            state_point = kf_noDe.get_x();
                            auto P_ = kf_noDe.get_P();
                            // std::cout << "P_ = " << P_ << std::endl;
                            // get the first <6,6> block of the covariance matrix
                            Eigen::Matrix<double, 6, 6> P_first = P_.block<6, 6>(0, 0);
                            Eigen::Matrix<double, 6, 6> P_first_T = P_first.transpose();
