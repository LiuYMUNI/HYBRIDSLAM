...
void h_share_model_stereo(state_ikfom &s, esekfom::dyn_share_datastruct<double> &ekfom_data)
{
    ...
    for (int i =0; i < mvl_observations.size(); i++)
    {
        ...
        Eigen::Matrix<double, 2, 6> Jac_final = -JacGood * SE3deriv;
    ...
    for (int i =0; i < mvr_observations.size(); i++)
    {
        ...
        // PC is the left camera coordinate of the current map point
        Eigen::Vector3d Pc = Rcw * Pw + tcw;
        // v3D is the right camera coordinate of the current map point
        Eigen::Vector3d v3D = rotation*Pc + translation;
        Eigen::Vector3d X_l = Pc;
        double x_w = X_l[0];
        double y_w = X_l[1];
        double z_w = X_l[2];
        ...
        JacGood(0, 2) = -Parameter0 * fd * v3D[0] / (r2 + z2);
        JacGood(1, 2) = -Parameter1 * fd * v3D[1] / (r2 + z2);
        Eigen::Matrix<double, 2, 6> Jac_final = -JacGood *rotation*SE3deriv;
        ekfom_data.h_x.block<2, 6>(2*i, 0) = Jac_final;
        // calculate the residual vector 2*1
...
//            0            0            0            1
    ...
    rotation_l << 0.999996, 0.00197641, 0.00213298,
                -0.00193759, 0.999835, -0.0180549,
                -0.00216832, 0.0180507, 0.999835;
    rotation = rotation_l;
    Eigen::Vector3d translation_l(-0.0916392, -0.000265604, -0.000402314);
    translation = translation_l;
...
//  -0.280637
    ...
    // set the translation part of Tleftb to zero
    Tleftb.translation() = Eigen::Vector3d(0, 0, 0);
    /*** debug record ***/
    FILE *fp;
...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight)
{
    ...
    else
    {
        ...
        mvl_WorldCoords_imucoor.clear();
        
        mvr_WorldCoords_imucoor.clear();
        ...
        // get the Tleftb rotation part
        Tleftb_rotation = Tleftb.so3().matrix();
    ...
    Eigen::Matrix3d Rcw_i = Tcw.so3().matrix();
    Rcw_i = Rcw_i * Tleftb_rotation;

    Eigen::Vector3d tcw_i = Tcw.translation();
    tcw_i = Tleftb_rotation * tcw_i;
