    ...
    while(status)
    {
        ...
        if(lidar_front <= tImLeft)
        {
            ...
            if(sync_packages(Measures_)) 
            {
                ...
                // before changing ikdtree, update the IESEKF first using the current ikdtree
                ...
                if(blast_LoopDetected && !bLoopDetected)
                {
                    ...
                    if(btreechanged)
                    {
                        loopchangedonce = false;
                        loopicpedonce = false;
                        std::cout << "btreechanged = true" << std::endl;
                    }

                }

                if(!loopicpedonce && nLoopBuffer > 0 &&btreechanged)
                {
                    loopicpedonce = true;
                    bool bRefine = false;
        ...
        else
        {
            ...
            if(!p_imu->get_InitFlag())
            {
                ...
                else
                {
                    ...
                    {
                        if(tImLeft - dMatchedTime < 60.0)
                        {
                            nLoopBuffer = 0;
                            std::cout << "small loop detected " << std::endl;
                        }
                        else
                        {
