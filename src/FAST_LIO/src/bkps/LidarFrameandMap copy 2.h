...
public:
    ...
    KD_TREE<pcl::PointXYZINormal>::Ptr ikdtreeptr_;
...
public:
    ...
    {
        // loop through all submaps, check if the submap has a frame with the matched time
        // if yes, set the submap as current submap
        ...
        for (const auto& submap : submaps_) {
            ...
            for (const auto& frame : submap->getPointCloudFrames()) {
                if(fabs(frame.getImgTimestamp() - dMatchedTime) < 0.1)
                {
                    ...
                    std::cout << "current submap change" << std::endl;
                    currentSubmap_ = submap;
                    std::cout << "current submap changed" << std::endl;
                    // safely change the ikdtree pointer to the new submap ikdtree pointer
