                ...
                if(degenerate_index_mean<degenerate_index_mean_thres)
                {
                    if(!bLoopDetected)
                    {
                        if(nLoopBuffer > 0)
                        {
                            ...
                            rmse = 0.0f;
                            // get the ptr of ikdtree
                            std::shared_ptr<KD_TREE<PointType>> ikdtree_ptr = std::make_shared<KD_TREE<PointType>>(ikdtree);

                            bRefine = refine_with_pt2pt_icp(feats_down_body, 0.12, 20, ikdtree_ptr, Ricp, ticp, state_point,rmse);
