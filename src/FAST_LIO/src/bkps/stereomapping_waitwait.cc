...
Sophus::SE3d Tcw;
Sophus::SE3d Twc;
Eigen::Matrix3d Riwi_g;
Eigen::Vector3d tiwi_g;
...
void h_share_model_stereo_prior(state_ikfom &s, esekfom::dyn_share_datastruct<double> &ekfom_data)
{
    ...
    // create a error vector to store the error of the current observation
    Eigen::VectorXd error_gg(2*total_observations);
    ...
    Eigen::JacobiSVD<Eigen::Matrix3d> svd_g(Riwi_g, Eigen::ComputeFullU | Eigen::ComputeFullV);
    Eigen::Matrix3d Riwi_ortho_g = svd_g.matrixU() * svd_g.matrixV().transpose();
    Sophus::SE3d Tiwi_g(Riwi_ortho_g, tiwi_g);
    Sophus::SE3d Tiwi_(Riwi_ortho, tiwi);
    
    Sophus::SE3d Tcwi_g = Tleftb * Tiwi_g;

    Sophus::SE3d Tcwi_ = Tleftb * Tiwi_;
    // the current camera pose in the camera world coordinate
    Sophus::SE3d Tcwc = Tcwi_ * Tbleft;

    Sophus::SE3d Tcwc_g = Tcwi_g * Tbleft;
    Sophus::SE3d Tc_in_cw_g = Tcwc_g.inverse();
    ...
    Eigen::Matrix3d Rcw_u_g = Tc_in_cw_g.so3().matrix();
    Eigen::Vector3d tcw_u_g = Tc_in_cw_g.translation();
    
    int left_size = globalMeasures.mvl_observations.size();
    int right_size = globalMeasures.mvr_observations.size();
    ...
    for (int i =0; i < globalMeasures.mvl_observations.size(); i++)
    {
        ...
        const Eigen::Vector3d Pc = Rcw_u * Pw + tcw_u;
        const Eigen::Vector3d Pc_g = Rcw_u_g * Pw + tcw_u_g;

        // get body coordinate
        const Eigen::Vector3d Pbody = Tbleft.so3().matrix()*Pc + Tbleft.translation();
        const Eigen::Vector3d Pbody_g = Tbleft.so3().matrix()*Pc_g + Tbleft.translation();

        Eigen::Vector3d v3D = Pc;
        Eigen::Vector3d v3D_g = Pc_g;
        double x = Pbody(0);
        double y = Pbody(1);
        double z = Pbody(2);
        double x_g = Pbody_g(0);
        double y_g = Pbody_g(1);
        double z_g = Pbody_g(2);
        ...
        // calculate the residual vector _g
        const double x2_plus_y2_g = v3D_g[0] * v3D_g[0] + v3D_g[1] * v3D_g[1];
        const double theta_g = atan2f(sqrtf(x2_plus_y2_g), v3D_g[2]);
        const double psi_g = atan2f(v3D_g[1], v3D_g[0]);

        const double theta2_g = theta_g * theta_g;
        const double theta3_g = theta_g * theta2_g;
        const double theta5_g = theta3_g * theta2_g;
        const double theta7_g = theta5_g * theta2_g;
        const double theta9_g = theta7_g * theta2_g;
        const double r_g = theta_g + Parameter4 * theta3_g + Parameter5 * theta5_g
                        + Parameter6 * theta7_g + Parameter7 * theta9_g;

        Eigen::Vector2d res;
        res[0] = Parameter0 * r_ * cos(psi) + Parameter2;
        res[1] = Parameter1 * r_ * sin(psi) + Parameter3;

        Eigen::Vector2d res_g;
        res_g[0] = Parameter0 * r_g * cos(psi_g) + Parameter2;
        res_g[1] = Parameter1 * r_g * sin(psi_g) + Parameter3;
        Eigen::Matrix<double, 2, 1> error = globalMeasures.mvl_observations[i] - res;
        Eigen::Matrix<double, 2, 1> error_g = globalMeasures.mvl_observations[i] - res_g;
        ...
        if(activaterobust){
            ...
            error_gg[2*i] = error_g[0];
            error_gg[2*i + 1] = error_g[1];
    ...
    for (int i =0; i < globalMeasures.mvr_observations.size(); i++)
    {
        ...
        Sophus::SE3d Trc_in_cw_g;
        Trc_in_cw_g = Trightleft * Tc_in_cw_g;
        Eigen::Matrix3d Rcw = Trc_in_cw.so3().matrix();
        Eigen::Vector3d tcw = Trc_in_cw.translation();
        Eigen::Matrix3d Rcw_g = Trc_in_cw_g.so3().matrix();
        Eigen::Vector3d tcw_g = Trc_in_cw_g.translation();
        // calculate the camera coordinate of the current map point
        ...
        Eigen::Vector3d Pc_g = Rcw_g * Pw + tcw_g;
        ...
        const Eigen::Vector3d Pbody_g = Tbright.so3().matrix()*Pc_g + Tbright.translation();
        Eigen::Vector3d X_l = Pbody;
        Eigen::Vector3d X_l_g = Pbody_g;
        Eigen::Vector3d v3D = Pc;
        Eigen::Vector3d v3D_g = Pc_g;
        double x_w = X_l[0];
        double y_w = X_l[1];
        double z_w = X_l[2];
        double x_w_g = X_l_g[0];
        double y_w_g = X_l_g[1];
        double z_w_g = X_l_g[2];
        ...
        Eigen::Matrix<double, 2, 1> error_g = globalMeasures.mvr_observations[i] - res_g;
        ...
        if(activaterobust){
            ...
            error_gg[2*i + 2*left_size] = error_g[0];
            error_gg[2*i + 1 + 2*left_size] = error_g[1];
            // errors.push_back(weighted_error);
            // jacobians.push_back(Jac_final);
        }
        else{
    ...
    double totalerror = 0;
    // define a row vector to be the transpose of the ekfom_data.h
    Eigen::RowVectorXd h_transpose = ekfom_data.h.transpose();
    // calculate the total error
    totalerror = h_transpose * h_transpose.transpose();
    std::cout << "totalerror unchange: " << totalerror << std::endl; 
    // get the dot product of the error_gg and the error_gg
    double totalerror_gg = error_gg.transpose() * error_gg;
    std::cout << "totalerror_gg: " << totalerror_gg << std::endl;
    // ekfom_data.h_x.block<3, 3>(2*right_size + 2*left_size, 0) = Eigen::Matrix3d::Identity();
    // ekfom_data.h_x.block<3, 3>(2*right_size + 2*left_size + 3, 3) = Eigen::Matrix3d::Identity(); 
    // ekfom_data.h_x.block<23, 23>(2*right_size + 2*left_size, 0) = Eigen::Matrix<double, 23, 23>::Identity();
    // ekfom_data.h[2*right_size + 2*left_size] = 0;
    // ekfom_data.h[2*right_size + 2*left_size + 1] = 0;
    // ekfom_data.h[2*right_size + 2*left_size + 2] = 0;
    // ekfom_data.h[2*right_size + 2*left_size + 3] = 0;
    // ekfom_data.h[2*right_size + 2*left_size + 4] = 0;
    // ekfom_data.h[2*right_size + 2*left_size + 5] = 0;
...
// void h_share_model_stereobkp(state_ikfom &s, esekfom::dyn_share_datastruct<double> &ekfom_data)
// {
//     // this function should use the input of visual map points, camera pose, then calculate the corresponding Jacobian matrix H and residual vector
...
//     Sophus::SE3d Tcwc = Tcwi_ * Tbleft;
//     Tcwc = Tcwc.inverse();
//     Eigen::Matrix3d Rcw_u = Tcwc.so3().matrix();
//     Eigen::Vector3d tcw_u = Tcwc.translation();
//     // std::cout << "Rcw_u: " << Rcw_u << std::endl;
//     // std::cout << "tcw_u: " << tcw_u << std::endl;
...
//         // get the camera coordinate of current map point
//         // extract the rotation matrix and the translation vector from the current camera pose Tcw( Sophus::SE3f)
//         const Eigen::Matrix3d Rcw = Tcwc.so3().matrix();
//         const Eigen::Vector3d tcw = Tcwc.translation();
//         // calculate the camera coordinate of the current map point
//         const Eigen::Vector3d Pw = globalMeasures.mvl_WorldCoords[i];
...
//     {
//         // caution!!! the right camera pose is not the same as the left camera pose
//         Eigen::Matrix3d Rcw = rotation_rl * Tcwc.so3().matrix();
//         Eigen::Vector3d tcw = rotation_rl * Tcwc.translation() + translation_rl;
//         // calculate the camera coordinate of the current map point
//         Eigen::Vector3d Pw = globalMeasures.mvr_WorldCoords[i];
//         // get the right camera coordinate of the current map point
//         Eigen::Vector3d Pc = Rcw * Pw + tcw;
...
//         JacGood(1, 2) = -Parameter1 * fd * v3D[1] / (r2 + z2);
//         Eigen::Matrix<double, 2, 6> Jac_final = -JacGood * Trightb.so3().matrix() * SE3deriv;
//         ekfom_data.h_x.block<2, 6>(2*i + 2*globalMeasures.mvl_observations.size(), 0) = Jac_final;
//         // calculate the residual vector 2*1
...
//         Eigen::Matrix<double, 2, 1> error = globalMeasures.mvr_observations[i] - res;
//         // std::cout << "right error: " << error << std::endl;
//         ekfom_data.h[2*i + 2*globalMeasures.mvl_observations.size()] = error[0];
//         ekfom_data.h[2*i + 1 + 2*globalMeasures.mvl_observations.size()] = error[1];
//         // std::cout << "right error: " << error[0] << " " << error[1] << std::endl;
//     }   
//     // solve_time += omp_get_wtime() - solve_start_;
// }
...
// void checkerror_bkp(Sophus::SE3d Tiwi_)
// {
//     // this function should use the input of visual map points, camera pose, then calculate the corresponding Jacobian matrix H and residual vector
//     int total_observations = mvl_observations.size() + mvr_observations.size();
//     // std::cout << "total_observations: " << total_observations << std::endl;
//     // get the current imu pose in imu world coordinate
...
//     // std::cout << "Rcw_u: " << Rcw_u << std::endl;
//     // std::cout << "tcw_u: " << tcw_u << std::endl;
//     Sophus::SE3d Tc_cw_ = Tcwc.inverse();
//     std::cout << "into left loops " << std::endl;
//     // cout the size of the observations
...
//     // for each observation, calculate the Jacobian matrix H and residual vector
//     for (int i =0; i < mvl_observations.size(); i++)
//     {

//         // get the camera coordinate of current map point
//         // extract the rotation matrix and the translation vector from the current camera pose Tcw( Sophus::SE3f)
        
//         const Eigen::Matrix3d Rcw = Tc_cw_.so3().matrix();
//         const Eigen::Vector3d tcw = Tc_cw_.translation();
//         // calculate the camera coordinate of the current map point
//         const Eigen::Vector3d Pw = mvl_WorldCoords[i];
...
//         res[0] = Parameter0 * r_ * cos(psi) + Parameter2;
//         res[1] = Parameter1 * r_ * sin(psi) + Parameter3;
//         Eigen::Matrix<double, 2, 1> error = mvl_observations[i] - res;
//         std::cout << "fastlio error: " << error[0] << std::endl;
//         std::cout << "fastlio error: " << error[1] << std::endl;
//         // ekfom_data.h[2*i] = error[0];
//         // ekfom_data.h[2*i + 1] = error[1];
//         // std::cout << "left error: " << error[0] << " " << error[1] << std::endl;
//     }
...
//     for (int i =0; i < mvr_observations.size(); i++)
//     {
//         // caution!!! the right camera pose is not the same as the left camera pose
//         Eigen::Matrix3d Rcw = rotation_rl * Tc_cw_.so3().matrix();
//         Eigen::Vector3d tcw = rotation_rl * Tc_cw_.translation() + translation_rl;
//         // calculate the camera coordinate of the current map point
//         Eigen::Vector3d Pw = mvr_WorldCoords[i];
//         // get the right camera coordinate of the current map point
//         Eigen::Vector3d Pc = Rcw * Pw + tcw;
...
//         JacGood(1, 2) = -Parameter1 * fd * v3D[1] / (r2 + z2);
//         Eigen::Matrix<double, 2, 6> Jac_final = JacGood * Trightb.so3().matrix() * SE3deriv;
//         // ekfom_data.h_x.block<2, 6>(2*i + 2*mvl_observations.size(), 0) = Jac_final;
//         // calculate the residual vector 2*1
...
//         Eigen::Matrix<double, 2, 1> error = mvr_observations[i] - res;
//         std::cout << "fastlio error: " << error[0] << std::endl;
//         std::cout << "fastlio error: " << error[1] << std::endl;
//         // std::cout << "right error: " << error << std::endl;
//         // ekfom_data.h[2*i + 2*mvl_observations.size()] = error[0];
//         // ekfom_data.h[2*i + 1 + 2*mvl_observations.size()] = error[1];
...
// void checkerror(Sophus::SE3d Tiwi_)
// {
//     // this function should use the input of visual map points, camera pose, then calculate the corresponding Jacobian matrix H and residual vector
//     int total_observations = mvl_observations.size() + mvr_observations.size();
//     // std::cout << "total_observations: " << total_observations << std::endl;
...
//  -0.280637
    ...
    while (status)
    {
        ...
        double frame_time_;
        {
            ...
            {
                frame_time_ = frame_time_queue.front();
        ...
        if(dataStore.hasData(frame_time_))
        {
            ...
            if(sync_packages_stereo(Measures_, frame_time_))
            {
                ...
                Eigen::Matrix3d Riwi = globalMeasures.Tiwi.so3().matrix();
                Eigen::Vector3d tiwi = globalMeasures.Tiwi.translation();
                Riwi_g = Riwi;
                tiwi_g = tiwi;
                ...
                kf.update_iterated_dyn_share_modified_stereo_error(globalMeasures.mean, state_point, solve_H_time);
...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight)
{
    ...
    else
    {
        ...
        // std::vector<double> prior_invSigma2s(23, 10000.0);
