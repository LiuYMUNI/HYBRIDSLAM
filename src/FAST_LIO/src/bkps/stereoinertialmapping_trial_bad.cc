  ...
  while(status)
  {
    ...
    
    {
        ...
        if(!bVIBA2)
        {
            ...
            {
                frame_time_g = lidar_time_queue.front();
    ...
    if(dataStore.hasData(frame_time_g))
    {
       data_ready = true;
  ...
  while(1)
  {
    ...
    //   }
      ...
      if(bVIBA2)
      {
        ...
        if(dataStore.hasData(frame_time))
        {
          std::cout << "data added" << std::endl;
        }
        else
        {
          std::cout << "data added failed" << std::endl;
        }
      }

      // Sophus::SE3f Twg = mpSLAM->mTwg;
      // std::cout << "Twg: " << Twg.matrix() << std::endl;

      Sophus::SE3d Twc = Tcw_.inverse();
      Eigen::Vector3d t = Tcw_.translation();
      Eigen::Vector3d tw = Twc.translation();
