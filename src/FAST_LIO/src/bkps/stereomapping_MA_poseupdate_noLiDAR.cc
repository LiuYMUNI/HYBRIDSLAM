...
void publish_odometry(const ros::Publisher & pubOdomAftMapped)
{
    ...
    odomAftMapped.header.stamp = ros::Time().fromSec(global_frametime);// ros::Time().fromSec(lidar_end_time);
...
void h_share_model_stereo_prior(state_ikfom &s, esekfom::dyn_share_datastruct<double> &ekfom_data)
{
    ...
    // ekfom_data.h_x = MatrixXd::Zero(2*total_observations + 6 , 12); //23
    // ekfom_data.h.resize(2*total_observations + 6);
    // ekfom_data.h_x = MatrixXd::Zero(23 , 23); //23
    ...
    //         if(abs(weighted_error[0]) > 100 || abs(weighted_error[1]) > 100)
    //         {
    //             weighted_error = 0.01 * weighted_error;
    //         }
    //         else if(abs(weighted_error[0]) > 10 || abs(weighted_error[1]) > 10)
    //         {
...
//  -0.280637
    ...
    SLAM.SaveKeyFrameTrajectoryTUM("/home/uas/b_KeyFrameTrajectory_TUM_Format.txt");
    SLAM.SaveTrajectoryTUM("/home/uas/b_FrameTrajectory_TUM_Format.txt");
