...
int main(int argc, char **argv)
{
  ...
  // Create SLAM system. It initializes all system threads and gets ready to process frames.
    ...
    while(status)
    {
        ...
        if (flg_exit) break;
        std::cout << "flg_exit is false" << std::endl;
        {
            if(!frame_time_queue.empty() && !lidar_time_queue.empty())
            {
                std::cout << "frame_time_queue and lidar_time_queue is not empty" << std::endl;
        ...
        if(globalMeasures.frame_or_not)
        {
            ...
            else
            {
                ...
                if(sync_packages(Measures_))
                {
                    ...
                    if(global_map.current_ikdtreeptr_ == nullptr)
                    {
                        ...
                        if(!lidarPRO.hasData(frame_time_g))
                        {
                            LidarProcess lidarprocess;
                            lidarprocess.lidarprocessed = true;
                            lidarprocess.feats_down_body_fororb = NULL;
            ...
            // check if the lidarPRO has the data of frame_time_g
            if(!lidarPRO.hasData(frame_time_g))
            {
                LidarProcess lidarprocess;
                lidarprocess.lidarprocessed = true;
        ...
        if (!imgLeftBuf.empty() && !imgRightBuf.empty())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Sopus::SE3f Tcw = mpSLAM->TrackStereo(imLeft,imRight,tImLeft);
                    Eigen::Matrix3d Rcw = Tcw.so3().matrix().cast<double>();
                    Eigen::Vector3d tcw = Tcw.translation().cast<double>();
                    Eigen::JacobiSVD<Eigen::Matrix3d> svd_cw(Rcw, Eigen::ComputeFullU | Eigen::ComputeFullV);
                    Eigen::Matrix3d Rcw_orthogonal = svd_cw.matrixU() * svd_cw.matrixV().transpose();
