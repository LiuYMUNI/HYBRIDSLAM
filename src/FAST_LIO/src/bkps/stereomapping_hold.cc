...
bool sync_packages_stereo(MeasureGroup &meas, double frame_time)
{
    ...
    while ((!imu_buffer.empty()) && (imu_time < frame_time))
    {
        ...
        //     std::cout << "IMU time: " << imu_time << " is larger than lidar time: " << lidar_end_time << std::endl;
        //     break;
        // }
...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight)
{
    ...
    Eigen::Matrix3d Rcw_i = Tiwi.so3().matrix();

    Eigen::Vector3d tcw_i = Tiwi.translation();
    ...
    Eigen::Quaterniond quaternion(Rcw_i);
    state_point.rot = MTK::SO3<double>(quaternion.coeffs());
    state_point.pos = tcw_i;
    ...
    kf.update_iterated_dyn_share_modified_stereo(LASER_POINT_COV, solve_H_time);
    state_point = kf.get_x();
    euler_cur = SO3ToEuler(state_point.rot);
    // std::cout << "Euler Angles: " << euler_cur << std::endl;
    state_pos = state_point.pos;
