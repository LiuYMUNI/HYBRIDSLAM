...
Eigen::Matrix3d rotation_rl;
Eigen::Vector3d translation_rl;
...
int    scan_num = 0;
bool sync_packages_stereo(MeasureGroup &meas, double frame_time)
{
    if (imu_buffer.empty()) {
        return false;
    }
    ...
    //     return false;
    // }
    meas.lidar_beg_time = frame_time;
    meas.lidar_end_time = frame_time;
    /*** push imu data, and pop from imu buffer ***/
    double imu_time = imu_buffer.front()->header.stamp.toSec();
    meas.imu.clear();
    while ((!imu_buffer.empty()) && (imu_time < frame_time))
    {
...
int main(int argc, char **argv)
{
    ...
    kf.init_dyn_share(get_f, df_dx, df_dw, h_share_model_stereo, NUM_MAX_ITERATIONS, epsi);
...
//            0            0            0            1
    ...
    rotation_rl << 0.999996, 0.00197641, 0.00213298,
                -0.00193759, 0.999835, -0.0180549,
                -0.00216832, 0.0180507, 0.999835;

    translation_rl<<-0.0916392, -0.000265604, -0.000402314;
...
//  -0.280637
    ...
    Tbright = Sophus::SE3d(R_orthogonal_r, translation_imu_right);
    ...
    message_filters::Subscriber<sensor_msgs::Image> left_sub(nh, "/uas_cam2/image", 1);
    message_filters::Subscriber<sensor_msgs::Image> right_sub(nh, "/uas_cam4/image", 1);
...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight)
{
    ...
    else
    {
        ...
        mvl_WorldCoords.clear();
        
        mvr_WorldCoords.clear();
        mvl_observations.clear();
        mvr_observations.clear();
        ...
        Twi = Twc*Tleftb;
        Tiwi = Tbleft*Twi;
        Sophus::SE3d Tiwc = Tiwi * Tbleft; 
        Sophus::SE3d Tcwc = Tleftb * Tiwc;
        // std::cout << "after transformation Tcwc = " << std::endl;
        // std::cout << Tcwc.so3().matrix() << std::endl;
        // std::cout << Tcwc.translation() << std::endl;
        Sophus::SE3d Tc_cw = Tcwc.inverse();
        std::cout << "after transformation Tc_cw = " << std::endl;
        std::cout << Tc_cw.so3().matrix() << std::endl;
        std::cout << Tc_cw.translation() << std::endl;
        ...
        mean = 1.0 / mean;
        // std cout the length of mvl_invSigma2s
        // std::cout << "mvl_invSigma2s.size() = " << mvl_invSigma2s.size() << std::endl;
        

        // apply transformation between imu and left camera to get the imu world coordinates of these map points
        // for (int i = 0; i < mvl_WorldCoords.size(); i++)
        // {
        //     mvl_WorldCoords_imucoor.push_back(rotation_imu_left * mvl_WorldCoords[i] + translation_imu_left);
        // }
        // for (int i = 0; i < mvr_WorldCoords.size(); i++)
        // {
        //     mvr_WorldCoords_imucoor.push_back(rotation_imu_left * mvr_WorldCoords[i] + translation_imu_left);
        // }
        // log the size of these vectors
        // std::cout << "mvl_WorldCoords.size() = " << mvl_WorldCoords.size() << std::endl;
        // std::cout << "mvl_observations.size() = " << mvl_observations.size() << std::endl;
        // std::cout << "mvr_WorldCoords.size() = " << mvr_WorldCoords.size() << std::endl;
        // std::cout << "mvr_observations.size() = " << mvr_observations.size() << std::endl;
        ...
        // for (int i = 0; i < mvl_WorldCoords.size(); i++)
        // {
        //     std::cout << "mvl_WorldCoords[" << i << "] = " << std::endl;
        //     std::cout << mvl_WorldCoords[i] << std::endl;
        // }
