...
double global_measuretime;
int global_iterCount;
...
public:
    ~PoseManager() {
        savePoseDataToFile("/home/uas/pose_data.txt");
    }

    void registerPose(double timestamp, double residualreduced, double initialresidual, int pointnum) {
        PoseData data;
        data.timestamp = timestamp;
        data.residualreduced = residualreduced;
        data.initialresidual = initialresidual;
...
private:
    ...
    void savePoseDataToFile(const std::string &filename) {
        std::ofstream file(filename);
        if (file.is_open()) {
            for (const auto &data : pose_data) {
                file << std::fixed << std::setprecision(6) << data.timestamp << " " << data.residualreduced << " " << data.initialresidual << " " << data.pointnum  << "\n";
            }
            file.close();
        } else {
...
void publish_odometry(const ros::Publisher &pubOdomAftMapped)
{
    ...
    odomAftMapped.header.stamp = ros::Time().fromSec(global_measuretime); // ros::Time().fromSec(lidar_end_time);
...
void publish_path(const ros::Publisher pubPath)
{
    ...
    msg_body_pose.header.stamp = ros::Time().fromSec(global_measuretime);
...
void h_share_model_stereo_prior(state_ikfom &s, esekfom::dyn_share_datastruct<double> &ekfom_data)
{
    ...
    // create a 2*total_observations vector to store the residual error
    Eigen::VectorXd residuals(2 * total_observations);
    ...
    for (int i = 0; i < globalMeasures.mvl_observations.size(); i++)
    {
        ...
        const double r_ = theta_ + Parameter4 * theta3_ + Parameter5 * theta5_ + Parameter6 * theta7_ + Parameter7 * theta9_;
        ...
        if (activaterobust)
        {
            ...
            residuals[2 * i] = weighted_error[0];
            residuals[2 * i + 1] = weighted_error[1];
    ...
    for (int i = 0; i < globalMeasures.mvr_observations.size(); i++)
    {
        ...
        Eigen::Vector3d Pw_kf = Tbleft.so3().matrix() * Pw + Tbleft.translation();
        // get the right camera coordinate of the current map point
        Eigen::Vector3d Pc = Rcw * Pw_kf + tcw;
        ...
        if (activaterobust)
        {
            ...
            residuals[2 * i + 2 * left_size] = weighted_error[0];
            residuals[2 * i + 1 + 2 * left_size] = weighted_error[1];
            // errors.push_back(weighted_error);
            // jacobians.push_back(Jac_final);
        }
        else
        ...
        // compute the dot product of the vector residuals, get the sum of the residuals
    ...
    double sum_residuals = residuals.dot(residuals);
    // std::cout << "sum_residuals: " << sum_residuals << std::endl;
    if(global_iterCount == 0)
    {
        firstresidual = sum_residuals;
    }
    global_iterCount++;
    lastresidual = sum_residuals;
...
int main(int argc, char **argv)
{
    ...
    while (status)
    {
        ...
        if (dataStore.hasData(frame_time_))
        {
            ...
            if (sync_packages_stereo(Measures_, frame_time_))
            {
                ...
                int total_observations = globalMeasures.mvl_observations.size() + globalMeasures.mvr_observations.size();
                double residualreduced =firstresidual - lastresidual;
                global_iterCount = 0;
                std::cout << "residualreduced: " << residualreduced << std::endl;
                poseManager.registerPose(frame_time_, residualreduced, firstresidual, total_observations);
                ...
                global_measuretime = frame_time_;
    ...
    SLAM.SaveKeyFrameTrajectoryTUM("/home/uas/a_KeyFrameTrajectory_TUM_Format.txt");
    SLAM.SaveTrajectoryTUM("/home/uas/a_FrameTrajectory_TUM_Format.txt");
    SLAM.SaveTrajectoryKITTI("/home/uas/a_FrameTrajectory_KITTI_Format.txt");
