...
void h_share_model_stereo_prior(state_ikfom &s, esekfom::dyn_share_datastruct<double> &ekfom_data)
{
    if(globalMeasures.frame_or_not)
    {
        ...
        if (effct_feat_num < 1)
        {
            ...
            std::cout << "No Effective Points h_share! \n";
...
int main(int argc, char **argv)
{
  ...
  while(status)
  {
    ...
    
    {
        ...
        {
            imgtime_g = frame_time_queue.front();
            lidartime_g = lidar_time_queue.front();
            if (imgtime_g < lidartime_g)
            {
                frame_time_g = imgtime_g;
                {
    ...
    {
        ros::spinOnce();
        std::cout << "waiting for data 0" << std::endl;
        // this thread sleep for 5ms
        status = ros::ok();
        // rate.sleep();
    ...
    if(globalMeasures.frame_or_not)
    {
        ...
        std::string frame_time_str = std::to_string(frame_time_g);
        std::cout << " syncing frame_time RRR = " << frame_time_str << std::endl;
        if(bfirst_0)
        {
           bfirst_0 = false;
           continue;
            ...
            if(globalMeasures.imupreprocessed)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            // kf.update_iterated_dyn_share_modified_stereo(0.00001, state_point, solve_H_time);
                            // when loop closed, do not update the state, just change the state
                            kf.change_x(state_point);
                        }
                    }
                }
                ...
                // std::cout << "state_point.pos2 !!! " << std::endl;

                pos_lid = state_point.pos + state_point.rot * state_point.offset_T_L_I;
                // std::cout << "pos_lid !!! " << std::endl;
                geoQuat.x = state_point.rot.coeffs()[0];
                geoQuat.y = state_point.rot.coeffs()[1];
                geoQuat.z = state_point.rot.coeffs()[2];
                geoQuat.w = state_point.rot.coeffs()[3];
                // std::cout << "geoQuat !!! " << std::endl;
                        ...
                        else
                        {
                            if(!bLoopDetected)
                            {
                                if(nLoopBuffer > 0 )
                                {
                                    ...
                                    // auto p_new = kf.get_P();
                                    // kf.change_x(state_point);
                                    if(nLoopBuffer < nLoopBuffer_t -3)
                                    {
                                        kf.update_iterated_dyn_share_modified_stereo(0.00001, state_point, solve_H_time);
                                    }
                                    else
        ...
        if(sync_packages(Measures_))
        {
            ...
            if (flg_first_scan)
            {
                ...
                lidarprocess.lidarprocessed = true;
                lidarprocess.feats_down_body_fororb = NULL;
                lidarprocess.bDegenerate = true;
                lidarprocess.dSum = 0.0;
                std::cout << "lidarprocess.dSum0 = " << lidarprocess.dSum << std::endl;
                ...
                lidarPRO.addData(frame_time_g, lidarprocess);
                continue;
            }
            double t0,t1,t2,t3,t4,t5,match_start, solve_start, svd_time;
            match_time = 0;
            ...
            if (feats_undistort->empty() || (feats_undistort == NULL))
            {
                ...
                lidarprocess.bDegenerate = true;
                lidarprocess.dSum = 0.0;
                std::cout << "lidarprocess.dSum1 = " << lidarprocess.dSum << std::endl;
                if(degenerate_indexs.size()>4)
                {
                    degenerate_indexs.pop_front();
                    degenerate_indexs.push_back(0.0);
            ...
            //   std::cout << "lasermap_fov_segment() succeed" << std::endl;
            downSizeFilterSurf.setInputCloud(feats_undistort);
            //   std::cout << "downSizeFilterSurf.setInputCloud(feats_undistort) succeed" << std::endl;
            downSizeFilterSurf.filter(*feats_down_body);
            //   std::cout << "downSizeFilterSurf.filter(*feats_down_body) succeed" << std::endl;
            //   pcl::PointCloud<pcl::PointXYZINormal>::Ptr feats_down_body_fororb(new pcl::PointCloud<pcl::PointXYZINormal>());
            PointCloudXYZI::Ptr feats_down_body_fororb(new PointCloudXYZI());
            pcl::copyPointCloud(*feats_down_body, *feats_down_body_fororb);
            t1 = omp_get_wtime();
            ...
            if(global_map.current_ikdtreeptr_ == nullptr)
            {
                ...
                if(global_map.current_ikdtreeptr_->Root_Node == nullptr)
                {
                    ...
                    lidarprocess.bDegenerate = true;
                    lidarprocess.dSum = 0.0;
                    std::cout << "lidarprocess.dSum2 = " << lidarprocess.dSum << std::endl;
                    if(degenerate_indexs.size()>4)
                    {
                        degenerate_indexs.pop_front();
                        degenerate_indexs.push_back(0.0);
            ...
            std::cout << "kdtree_size_st = " << kdtree_size_st << std::endl;
            ...
            if (feats_down_size < 5)
            {
                ...
                lidarprocess.bDegenerate = true;
                lidarprocess.dSum = 0.0;
                std::cout << "lidarprocess.dSum3 = " << lidarprocess.dSum << std::endl;
                if(degenerate_indexs.size()>4)
                {
                    degenerate_indexs.pop_front();
                    degenerate_indexs.push_back(0.0);
                ...
                if(degenerate_index_mean<degenerate_index_mean_thres)
                {
                    if(!bLoopDetected)
                    {
                        if(nLoopBuffer > 0)
                        {
                            ...
                            rmse = 0.0f;
                            bRefine = refine_with_pt2pt_icp(feats_down_body, 0.12, 20, global_map.current_ikdtreeptr_, Ricp, ticp, state_point,rmse);
                            ...
                            kf.update_iterated_dyn_share_modified(LASER_POINT_COV, solve_H_time);
                            // auto p_new = kf.get_P();
                            auto estimated_state = kf.get_x();
                            // Calculate the translation difference between the estimated state and the refined state
                            Eigen::Vector3d translation_difference = estimated_state.pos - state_point.pos;
                            double translation_difference_norm = translation_difference.norm();

                            // Calculate the rotation difference between the estimated state and the refined state
                            Eigen::Matrix3d estimated_rotation_matrix = estimated_state.rot.toRotationMatrix();
                            Eigen::Matrix3d refined_rotation_matrix = state_point.rot.toRotationMatrix();
                            double rotation_difference_norm = calculateRotationDifference(estimated_rotation_matrix, refined_rotation_matrix);

                            // Check if the translation and rotation differences are within the specified limits
                            if (rmse < 0.11) {
                                nLoopBuffer -= 10;
                            }

                            // Print the results
                            std::cout << "Translation difference norm: " << translation_difference_norm << std::endl;
                            std::cout << "Rotation difference norm: " << rotation_difference_norm << " degrees" << std::endl;
                            std::cout << "nLoopBuffer: " << nLoopBuffer << std::endl;
                ...
                else
                {
                    if(!bLoopDetected)
                    {
                        if(nLoopBuffer > 0 )
                        {
                            ...
                            // Check if the translation and rotation differences are within the specified limits translation_difference_norm < 0.3 && rotation_difference_norm < 20.0
                            if (rmse < 0.11) {
                                nLoopBuffer -= 10;
                            }
                            ...
                            //     // kf.update_iterated_dyn_share_modified(0.01, solve_H_time);
                            // }
                            // kf.update_iterated_dyn_share_modified(0.001, solve_H_time);
                        }
                        else
            ...
            consumed_time_entry.isLiDAR = true;
            consumed_time.push_back(consumed_time_entry);
            /*** add the feature points to map kdtree ***/
            t3 = omp_get_wtime();
            if(nLoopBuffer == 0 || rmse < 0.11)
            {
            ...
            {
                std::cout << "no adding points, nLoopBuffer = " << nLoopBuffer << std::endl;
            }
            ...
            LidarProcess lidarprocess;
            lidarprocess.lidarprocessed = true;
            lidarprocess.feats_down_body_fororb = feats_down_body_fororb;
            lidarprocess.bDegenerate = kf.getDegenerate();
            lidarprocess.dSum = kf.getSum();
            std::cout << "lidarprocess.dSum4 = " << lidarprocess.dSum << std::endl;
            ...
            int dSum_int = dSum;
            global_map.addPointCloudFrame(feats_down_body_fororb, frame_time_g, imgtime_g, dSum_int, Tiwi, state_point, PointToAdd_global, PointNoNeedDownsample_global, PointToAdd_body_global, PointNoNeedDownsample_body_global);
    ...
    while(1)
    {
        ...
        if (!imgLeftBuf.empty()&&!imgRightBuf.empty()&&!mpImuGb->imuBuf.empty())
        {
            ...
            if(!mpImuGb->imuBuf.empty())
            {
                ...
                std::cout << "imuBuf not empty" << std::endl;
                vImuMeas.clear();
                ...
                // if(vImuMeas.empty())
                // {
                //     std::cout << "vImuMeas empty" << std::endl;
                //     mpImuGb->mBufMutex.unlock();
                //     DataEntry Measures;
                //     Measures.frame_or_not = true;
                //     // Measures.Tiwi = Tiwi;
                //     Measures.imupreprocessed = false;
                //     Measures.validdata = false;
            ...
            // {
            //     bleftbset = true;
            //     mpSLAM->SetTleftb(Tleftb);
            // }
            double lidar_time_front = lidar_time_queue_for_vslam.front();
            std::cout << "lidar_time_front = " << lidar_time_front << std::endl;
            if(frame_time >= lidar_time_front)
            {
                if(bfirst)
                {
                    std::cout << " frame time > lidar front && bfirst = true" << std::endl;
                    bfirst = false;
                    while(frame_time >= lidar_time_front)
                    {
                        while(!lidarPRO.hasData(lidar_time_front))
                        {
                            // ////std::cout << "waiting for lidar to be processed" << std::endl;
                            // current thread sleep for 1ms
                            std::this_thread::sleep_for(std::chrono::milliseconds(1));
                        }
                        globalLidarProcess = lidarPRO.getData(lidar_time_front);
                        last_processed_lidartime = lidar_time_front;
                        ...
                        lidar_time_front = lidar_time_queue_for_vslam.front();
                    }
                    double t_track_start = omp_get_wtime();
                    Sophus::SE3f Tcw = mpSLAM->TrackStereo(imLeft,imRight,tImLeft,vImuMeas);
                    double t_track_end = omp_get_wtime();
                    Consumed_time consumed_time_entry;
                    consumed_time_entry.timestamp = ros::Time().fromSec(frame_time);
                    consumed_time_entry.time = t_track_end - t_track_start;
                    consumed_time_entry.isLiDAR = false;
                    consumed_time.push_back(consumed_time_entry);
                    int trackingstat = mpSLAM->mtrackingStat;
                    if(trackingstat == 2)
                    {
                        nTrackingstate += 2;
                    }
                    ...
                    {
                        ////std::cout << "bVIBA0 && !bVIBA1" << std::endl;
                        ...
                        Measures.Tiwi = Tiwi;
                        Measures.imupreprocessed = false;
                        Measures.validdata = true;
                        dataStore.addData(frame_time, Measures);

                    }
                    else if(bVIBA0 && bVIBA1 && !bVIBA2)
                    {
                        ////std::cout << "bVIBA0 && bVIBA1 && !bVIBA2" << std::endl;
                ...
                {
                    std::cout << "frame time is bigger than lidar time front && not bfirst" << std::endl;
                    while(frame_time >= lidar_time_front)
                    {
                        while(!lidarPRO.hasData(lidar_time_front))
                        {
                    ...
                    bVIBA0 = mpSLAM->mbInertialBA0;
                    bLoopDetected = mpSLAM->mbLoopDetected;
                    if(!blast_LoopDetected && bLoopDetected)
                    {
                        dMatchedTime = mpSLAM->mdMatchedTime;
                    ...
                    if(blast_LoopDetected && !bLoopDetected)
                    {
                        ...
                        global_map.changeCurrentSubmap(dMatchedTime);
                    }
                    std::cout << "bLoopDetected = " << bLoopDetected << std::endl;
                    if(bVIBA0)
                    {
                        
                        ////std::cout << "now the frame time is smaller than lidar time front "<< std::endl;
                        PointCloudXYZI::Ptr feats_down_body_fororbg(new PointCloudXYZI());
                        if(sync_packages_stereo(Measures_, frame_time))
                        {
                            ...
                            // cout << "sync_packages_stereo(Meausures, frame_time) succeed" << endl;
                            bool bfirstlocal = false;
                            if(flg_first_scan)
                            {
                                flg_first_scan = false;
                                p_imu->first_lidar_time = frame_time;
                                bfirstlocal = true;
                                feats_down_body_fororbg = NULL;
                            }
                            else
                            {
                                bfirstlocal = false;
                            ...
                            if(!bfirstlocal)
                            {
                                // process imu here because of PCL undistortion
                                p_imu->Process_stereo(Measures_, kf, feats_down_body_fororbg);
                            }
                        }
                        state_point = kf.get_x();
                        ...
                        // get the first <6,6> block of the covariance matrix
                        ...
                        // Eigen::Vector3f velocity_c0_f = velocity_c0.cast<float>();
                        std::cout << "velocity_c0 = " << velocity_c0 << std::endl;
                        Sophus::SE3f Tcw;
                        std::cout << "degenerate_index_mean = " << degenerate_index_mean << std::endl;
                        std::cout << "globalLidarProcess.dSum0 = " << globalLidarProcess.dSum << std::endl;
                        if(globalcounter > 0)
                        {
                            ...
                            {
                                double t_track_start = omp_get_wtime();
                                //global_track_stereo_pose_counter % 2 == 0
                                Tcw = mpSLAM->TrackStereo_Pose(imLeft,imRight,tImLeft,vImuMeas, "", Tc0bn, information_matrix, velocity_c0.cast<float>());
                                double t_track_end = omp_get_wtime();
                                Consumed_time consumed_time_entry;
                                consumed_time_entry.timestamp = ros::Time().fromSec(frame_time);
                                consumed_time_entry.time = t_track_end - t_track_start;
                                ...
                                if(!bLoopDetected)
                                {
                                    if(nLoopBuffer > 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            Tcw = mpSLAM->TrackStereo_Pose(imLeft,imRight,tImLeft,vImuMeas, "", Tc0bn, information_matrix);
                                        }
                                        // Tcw = mpSLAM->TrackStereo(imLeft,imRight,tImLeft,vImuMeas);
                                    }
                                    else
                        ...
                        int sizebeforetrackstereopcl;
                        if(feats_down_body_fororbg == NULL)
                        {
                            sizebeforetrackstereopcl = 0;
                        }
                        else
                        {
                            sizebeforetrackstereopcl = feats_down_body_fororbg->points.size();
                        }
                        ...
                        else if(bVIBA0 && !bVIBA1)
                        {
                            ...
                            Measures.frame_or_not = true;
                            Measures.Tiwi = Tiwi;
                            // std::cout << "Tiwi here = " << Tiwi.matrix() << std::endl;
                            Measures.imupreprocessed = true;
                            Measures.validdata = true;
                            dataStore.addData(frame_time, Measures);
                        ...
                        else if(bVIBA0 && bVIBA1 && !bVIBA2)
                        {
                            ...
                            Measures.frame_or_not = true;
                            Measures.Tiwi = Tiwi;
                            // std::cout << "Tiwi here 0= " << Tiwi.matrix() << std::endl;
                            Measures.imupreprocessed = true;
                            Measures.validdata = true;
                            dataStore.addData(frame_time, Measures);
                        ...
                        else if(bVIBA2)
                        {
                            ...
                            Tycn = Tcw_.inverse();
                            Tc0cn = Tc0y * Tycn;
                            // log the rotation matrix and translation vector of Tc0y 
                            // std::cout << "Rc0y = " << Tc0y.so3().matrix() << std::endl;
                            // std::cout << "tc0y = " << Tc0y.translation() << std::endl;
                            Twi = Tc0cn * Tleftb;
                            // std::cout << "Rleftb = " << Tleftb.so3().matrix() << std::endl;
                            // std::cout << "tleftb = " << Tleftb.translation() << std::endl;
                            Tiwi = Tb0c0*Twi;
                            // std::cout << "Rb0c0 = " << Tb0c0.so3().matrix() << std::endl;
                            // std::cout << "tb0c0 = " << Tb0c0.translation() << std::endl;
                            DataEntry Measures;
                            Measures.frame_or_not = true;
                            Measures.Tiwi = Tiwi;
                            // std::cout << "Tiwi here 1= " << Tiwi.matrix() << std::endl;
                            Measures.imupreprocessed = true;
                            Measures.validdata = true;
                            dataStore.addData(frame_time, Measures);
                        }
            ...
            {
                std::cout << "frame time is smaller than lidar time frontxx" << std::endl;
                if(bfirst)
                {
                    std::cout << "frame time < lidar front && bfirst is 1 " << std::endl;
                    bfirst = false;
                    Sophus::SE3f Tcw = mpSLAM->TrackStereo(imLeft,imRight,tImLeft,vImuMeas);
                    int trackingstat = mpSLAM->mtrackingStat;
                    if(trackingstat == 2)
                ...
                {
                    std::cout << " frame time < lidar front && not bfirst && last_processed_lidartime = 0.0" << std::endl;
                    
                    if(last_processed_lidartime == 0.0)
                    {
                        Sophus::SE3f Tcw = mpSLAM->TrackStereo(imLeft,imRight,tImLeft,vImuMeas);
                        int trackingstat = mpSLAM->mtrackingStat;
                        if(trackingstat == 2)
                    ...
                    else
                    {
                        ...
                        if(bVIBA0)
                        {
                            ////std::cout << "frame time < lidar front && not bfirst && last_lidar processed != 0.0 "<< std::endl;
                            PointCloudXYZI::Ptr feats_down_body_fororbg(new PointCloudXYZI());
                            if(sync_packages_stereo(Measures_, frame_time))
                            {
                                Measures_.lidar = globalLidarProcess.feats_down_body_fororb;
                            ...
                            Sophus::SE3d Tb0bn;
                            Tb0bn = Sophus::SE3d(R_b0bn, t);
                            std::cout << "Tb0bn at vslam after imu processing = " << Tb0bn.matrix() << std::endl;
                            int sizebeforetrackstereopcl;
                            if(feats_down_body_fororbg == NULL)
                            {
                                sizebeforetrackstereopcl = 0;
                            ...
                            ////std::cout << "sizebeforetrackstereopcl = " << sizebeforetrackstereopcl << std::endl;
                            // Sophus::SE3f Tcw = mpSLAM->TrackStereo_PCL(imLeft,imRight,tImLeft,vImuMeas,"",feats_down_body_fororbg);
                            Sophus::SE3f Tcw = mpSLAM->TrackStereo(imLeft,imRight,tImLeft,vImuMeas);
                            int trackingstat = mpSLAM->mtrackingStat;
                            if(trackingstat == 2)
                            {
