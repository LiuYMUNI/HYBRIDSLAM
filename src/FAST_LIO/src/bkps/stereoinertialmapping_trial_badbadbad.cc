...
bool sync_packages(MeasureGroup &meas)
{
    if (lidar_buffer.empty() || imu_buffer.empty()) {
        if(lidar_buffer.empty()) std::cout << "Lidar buffer empty" << std::endl;
        if(imu_buffer.empty()) std::cout << "IMU buffer empty" << std::endl;
        return false;
    }
    ...
    if(!lidar_pushed)
    {
        ...
        {
            std::cout << "lidar scan time too short" << std::endl;
            lidar_end_time = meas.lidar_beg_time + lidar_mean_scantime;
        }
        else
        {
            std::cout << "lidar scan time normal" << std::endl;
            scan_num ++;
            lidar_end_time = meas.lidar_beg_time + meas.lidar->points.back().curvature / double(1000);
            std::string lidar_beg_time = std::to_string(meas.lidar_beg_time);
    ...
    {
        std::cout << "last IMU time < lidar end time" << std::endl;
        return false;
    }
...
int main(int argc, char **argv)
{
  ...
  while(status)
  {
    ...
    {
        std::cout << "data_ready = " << data_ready << std::endl;
    ...
    while(!dataStore.hasData(frame_time_g))
    {
        ...
        ros::spinOnce();
        std::string frame_time_str = std::to_string(frame_time_g);
        std::cout << "waiting for frame_time = " << frame_time_str << std::endl;
        // this thread sleeps for 1ms
    ...
    if(globalMeasures.frame_or_not)
    {
        ...
        {
          std::cout << "sync_packages(Measures_) succeed" << std::endl;
          if (flg_first_scan)
          {
            ...
            std::string frame_time_str = std::to_string(frame_time_g);
            std::cout << "adding lidar data = " << frame_time_str << std::endl;
        ...
        {
            std::cout << "sync_packages(Measures_) failed" << std::endl;
            LidarProcess lidarprocess;
            lidarprocess.lidarprocessed = true;
            lidarprocess.feats_down_body_fororb = NULL;
            std::string frame_time_str = std::to_string(frame_time_g);
...
void ImageGrabber::SyncWithImu()
{
    ...
    while(1)
    {
        ...
        if (!imgLeftBuf.empty()&&!imgRightBuf.empty()&&!mpImuGb->imuBuf.empty())
        {
            ...
            {
                std::cout << "tImLeft > imuBuf.back()" << std::endl;
                continue;
            }
            this->mBufMutexLeft.lock();
            ...
            if(frame_time >= lidar_time_front)
            {
                if(bfirst)
                {
                    ...
                    {
                        std::cout << "waiting for lidar to be processed 3" << std::endl;
                        while(!lidarPRO.hasData(lidar_time_front))
                        {
                            std::cout << "waiting for lidar to be processed2" << std::endl;
                        ...
                        {
                            std::cout << "size of lidar_time_queue_for_vslam = " << lidar_time_queue_for_vslam.size() << std::endl;
                            {
                                std::lock_guard<std::mutex> lock(lidar_time_queue_for_vslam_mutex);
                        ...
                        else
                        {
                            ...
                            {
                                std::cout << "waiting for lidar_time_queue_for_vslam to be bigger than 1" << std::endl;
                                std::this_thread::sleep_for(std::chrono::milliseconds(1));
                            }
                        }
                ...
                else
                {
                    ...
                    while(frame_time >= lidar_time_front)
                    {
                        std::cout << "waiting for lidar to be processed 1" << std::endl;
                        while(!lidarPRO.hasData(lidar_time_front))
                        {
                            std::string lidar_time_str = std::to_string(lidar_time_front);
                            
                            std::cout << "waiting for lidar to be processed" << lidar_time_str << std::endl;
                            std::this_thread::sleep_for(std::chrono::milliseconds(1));
                        }
                        globalLidarProcess = lidarPRO.getData(lidar_time_front);
                        last_processed_lidartime = lidar_time_front;
                        // get the size of the lidar_time_queue_for_vslam
                        if(lidar_time_queue_for_vslam.size() >1)
                        {
                            std::cout << "size of lidar_time_queue_for_vslam = " << lidar_time_queue_for_vslam.size() << std::endl;
                            {
                    ...
                    if(false)
                    {
                        ...
                        {
                            std::cout << "sync_packages_stereo(Measures_, frame_time) failed" << std::endl;
                            Sophus::SE3f Tcw = mpSLAM->TrackStereo(imLeft,imRight,tImLeft,vImuMeas);
                            DataEntry Measures;
                            Measures.validdata = false;
                            dataStore.addData(frame_time, Measures);
