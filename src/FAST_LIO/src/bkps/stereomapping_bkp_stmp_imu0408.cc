...
int main(int argc, char **argv)
{
  ...
  // Create SLAM system. It initializes all system threads and gets ready to process frames.
    ...
    while(status)
    {
        ...
        if (!igb.imgLeftBuf.empty()&&!igb.imgRightBuf.empty()&&!igb.mpImuGb->imuBuf.empty())
        {
        ...
        igb.mBufMutexRight.lock();
        while ((tImLeft - tImRight) > maxTimeDiff && igb.imgRightBuf.size() > 1)
        {
            igb.imgRightBuf.pop();
            tImRight = igb.imgRightBuf.front()->header.stamp.toSec();
        }
        igb.mBufMutexRight.unlock();
        
        igb.mBufMutexLeft.lock();
        while ((tImRight - tImLeft) > maxTimeDiff && igb.imgLeftBuf.size() > 1)
        {
            igb.imgLeftBuf.pop();
            tImLeft = igb.imgLeftBuf.front()->header.stamp.toSec();
        }
        igb.mBufMutexLeft.unlock();
        if ((tImLeft - tImRight) > maxTimeDiff || (tImRight - tImLeft) > maxTimeDiff)
        {
            continue;
        }
        if(tImLeft>igb.mpImuGb->imuBuf.back()->header.stamp.toSec())
            continue;
        igb.mBufMutexLeft.lock();
        {
        ...
        if(lidar_front <= tImLeft)
        {
            ...
            if(sync_packages(Measures_)) 
            {
                ...
                // kf.update_iterated_dyn_share_modified_matrix(LASER_POINT_COV, solve_H_time);
        ...
        else
        {
            ...
            igb.mpImuGb->mBufMutex.lock();
            {
                if(!igb.mpImuGb->imuBuf.empty())
                {
                    vImuMeas.clear();
                    while(!igb.mpImuGb->imuBuf.empty() && igb.mpImuGb->imuBuf.front()->header.stamp.toSec()<=tImLeft)
                    {
                    double t = igb.mpImuGb->imuBuf.front()->header.stamp.toSec();
                    cv::Point3f acc(igb.mpImuGb->imuBuf.front()->linear_acceleration.x, igb.mpImuGb->imuBuf.front()->linear_acceleration.y, igb.mpImuGb->imuBuf.front()->linear_acceleration.z);
                    cv::Point3f gyr(igb.mpImuGb->imuBuf.front()->angular_velocity.x, igb.mpImuGb->imuBuf.front()->angular_velocity.y, igb.mpImuGb->imuBuf.front()->angular_velocity.z);
                    vImuMeas.push_back(ORB_SLAM3::IMU::Point(acc,gyr,t));
                    igb.mpImuGb->imuBuf.pop();
                    }
                }
            }
            igb.mpImuGb->mBufMutex.unlock();
            std::cout << "lidar_front > tImLeft" << std::endl;
            igb.mBufMutexLeft.lock();
            igb.imgRightBuf.pop();
            igb.mBufMutexLeft.unlock();
            ...
            if(!p_imu->get_InitFlag() && bVIBA0)
            {
                std::cout << "p_imu->get_InitFlag() && bVIBA0" << std::endl;
                ...
                else
                {
                    ...
                    if(!bLoopDetected)
                    {
                        ...
                        else
                        {
                            ...
                            // Tcw = SLAM.TrackStereo_Pose(imLeft,imRight,tImLeft,vImuMeas, "", Tc0bn, information_matrix, velocity_c0.cast<float>());         
                            if(true)
                            {
                                double t_track_start = omp_get_wtime();
                                Tcw = SLAM.TrackStereo_Pose(imLeft,imRight,tImLeft,vImuMeas, "", Tc0bn, information_matrix, velocity_c0.cast<float>());
                                double t_track_end = omp_get_wtime();
                                std::cout << "t_track_end - t_track_start = " << t_track_end - t_track_start << std::endl;
                            }
                    ...
                    std::cout << "kf.getSum() = "  << std::endl;
                }
            }
            else
            {
                std::cout << "p_imu->get_InitFlag() && bVIBA0 false" << std::endl;
                Sophus::SE3f Tcw = SLAM.TrackStereo(imLeft,imRight,tImLeft,vImuMeas);
