        ...
        if(globalMeasures.frame_or_not)
        {
            ...
            if(sync_packages(Measures_))
            {
                ...
                if (flg_first_scan)
                {
                    ...
                    VSLAMData vslamdata;
                    vslamdata.frame_or_not = false;
                    vslamdata.lidarprocessed = true;
                    vslamdata.feats_down_body_fororb = NULL;
                    vslamDataStore.addData(frame_time_g, vslamdata);
                    continue;
                }
                double t0,t1,t2,t3,t4,t5,match_start, solve_start, svd_time;
                match_time = 0;
                ...
                vslamdata.frame_or_not = false;
                vslamdata.lidarprocessed = true;
                vslamdata.feats_down_body_fororb = feats_down_body_fororb;
                vslamDataStore.addData(frame_time_g, vslamdata);
    ...
    while(1)
    {
        ...
        if (!imgLeftBuf.empty()&&!imgRightBuf.empty()&&!mpImuGb->imuBuf.empty())
        {
            ...
            }
            {
                ...
                frame_time_queue_for_vslam.push_back(frame_time);
            }
            ...
            std::cout << "preparing data for VSLAM" << std::endl;
            VSLAMData vslamdata;
            ...
            std::cout << "data prepared for VSLAM" << std::endl;
        }
    }
}
...
void ImageGrabber::ProcessData()
{
    while(1)
    {
        ...
        if (flg_exit) break;
        std::cout << "flg_exit = " << flg_exit << std::endl;
        if(!frame_time_queue_for_vslam.empty() && !lidar_time_queue_for_vslam.empty())
        {
            double frame_time = frame_time_queue_for_vslam.front();
            double lidar_time = lidar_time_queue_for_vslam.front();
            if (frame_time < lidar_time)
            {
            ...
            {
                frame_time_g_vslam = lidar_time;
                {
                    std::lock_guard<std::mutex> lock(lidar_time_queue_for_vslam_mutex);
                    lidar_time_queue_for_vslam.pop_front();
                }
            }
            std::string frame_time_str = std::to_string(frame_time_g_vslam);
            std::cout << "frame_vslam_timestr_ = " << frame_time_str << std::endl;
        }
        if(frame_time_g_vslam == 0.0)
        {
            std::cout << "frame_time_g_vslam = 0.0" << std::endl;
            continue;
        }
        else
        {
            std::cout << "frame_time_g_vslam = " << frame_time_g_vslam << std::endl;
            while(!vslamDataStore.hasData(frame_time_g_vslam))
            {
                continue;
            }
        }
        globalVSLAMData = vslamDataStore.getData(frame_time_g_vslam);
        if(globalVSLAMData.frame_or_not)
        {
            if(last_lidartime_vslam == 0.0)
            {
                bfirst =true;
                std::cout << "the first data is frame, there is no pcl ptr" << std::endl;
                Sophus::SE3f Tcw = mpSLAM->TrackStereo(globalVSLAMData.imLeft, globalVSLAMData.imRight, globalVSLAMData.tImLeft, globalVSLAMData.vImuMeas);
                if(!bTracker_ikdtree)
                {
                    if(bikdtree_built)
                    {
                ...
                if(!bVIBA0)
                {
                    ...
                    dataStore.addData(frame_time_g_vslam, Measures);
                }
            ...
            {
                std::cout << "the first data is lidar, there is pcl ptr" << std::endl;
                if(!bTracker_ikdtree)
                {
                    if(bikdtree_built)
                    {
                ...
                // get the last VSLAMData
                VSLAMData last_vslamdata = vslamDataStore.getData(last_lidartime_vslam);
                PointCloudXYZI::Ptr feats_down_body_fororbg(new PointCloudXYZI());
                if(sync_packages_stereo(Measures_, frame_time_g_vslam))
                {
                    Measures_.lidar = last_vslamdata.feats_down_body_fororb;
                    if(flg_first_scan)
                    {
                        flg_first_scan = false;
                        p_imu->first_lidar_time = frame_time_g_vslam;
                        bfirst =true;
                ...
                Sophus::SE3f Tcw = mpSLAM->TrackStereo_PCL(globalVSLAMData.imLeft, globalVSLAMData.imRight, globalVSLAMData.tImLeft, globalVSLAMData.vImuMeas, "",feats_down_body_fororbg);
                if(bVIBA0 && !bVIBA2)
                {
                    mpSLAM->CheckIKdtree_size();
                }
        ...
        {
            last_lidartime_vslam = frame_time_g_vslam;
            continue;
        }
