...
double last_sum;
double dMatchedTime;
// a global variable std::pair to store the times and the poses, which are double and Sophus::SE3f vector
std::pair <std::vector<double>, std::vector<Sophus::SE3f>> IESEKF_Poses;
...
PointCloudXYZI::Ptr feats_undistort_G(new PointCloudXYZI());

std::vector<double> times;
std::vector<state_ikfom> states;
std::vector<PointCloudXYZI::Ptr> pcls;
std::vector<sensor_msgs::PointCloud2> laserCloudmsgs;
...
void RGBpointBodyToWorld_STATE(PointType const * const pi, PointType * const po, state_ikfom s_in)
{
    V3D p_body(pi->x, pi->y, pi->z);
    V3D p_global(s_in.rot * (s_in.offset_R_L_I*p_body + s_in.offset_T_L_I) + s_in.pos);
...
void RGBpointBodyToWorld_LOOP(PointType const * const pi, PointType * const po, Sophus::SE3f& s)
{
    // convert SE3f to SE3d
    Sophus::SE3d s_d = s.cast<double>();
    V3D p_body(pi->x, pi->y, pi->z);
    V3D p_global( s_d.rotationMatrix() * (state_point.offset_R_L_I*p_body + state_point.offset_T_L_I) + s_d.translation());
...
void publish_frame_world(const ros::Publisher & pubLaserCloudFull)
{
    ...
    if (scan_pub_en)
    {
        ...
        PointCloudXYZI::Ptr laserCloudWorld_bkp( \
        ...
        sensor_msgs::PointCloud2 laserCloudmsg_bkp;
        pcl::toROSMsg(*laserCloudWorld_bkp, laserCloudmsg_bkp);
        laserCloudmsg_bkp.header.stamp = ros::Time().fromSec(lidar_end_time);
        laserCloudmsg_bkp.header.frame_id = "camera_init";

        sensor_msgs::PointCloud2 laserCloudmsg;
        pcl::toROSMsg(*laserCloudWorld, laserCloudmsg);
        laserCloudmsg.header.stamp = ros::Time().fromSec(lidar_end_time);
        laserCloudmsg.header.frame_id = "camera_init";
        // pcls.push_back(laserCloudWorld_bkp);
        feats_undistort_G = laserCloudWorld_bkp;
        laserCloudmsgs.push_back(laserCloudmsg_bkp);
...
void publish_frame_world_RE(const PointCloudXYZI::Ptr &pclin, const ros::Publisher & pubLaserCloudFull, state_ikfom state_in)
{
    if(scan_pub_en)
    {
        PointCloudXYZI::Ptr laserCloudFullRes(pclin);
        int size = laserCloudFullRes->points.size();
        PointCloudXYZI::Ptr laserCloudWorld( \
                        new PointCloudXYZI(size, 1));
        ...
        for (int i = 0; i < size; i++)
        {
            RGBpointBodyToWorld_STATE(&laserCloudFullRes->points[i], \
                                &laserCloudWorld->points[i], state_in);
        }
...
void publish_frame_world_MS(sensor_msgs::PointCloud2& laserCloudmsg, const ros::Publisher & pubLaserCloudFull, state_ikfom state_in)
{
    if(scan_pub_en)
    {
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher pubLaserCloudFull_LOOP = no.advertise<sensor_msgs::PointCloud2>
            ("/cloud_registered_LOOP", 100000);
  ...
  // Create SLAM system. It initializes all system threads and gets ready to process frames.
    ...
    while(status)
    {
        ...
        if(lidar_front <= tImLeft)
        {
            ...
            if(sync_packages(Measures_)) 
            {
                ...
                std::cout << "frameTimes_IESEKFsize():"<< frameTimes_IESEKF.size()<< std::endl;
                ...
                state_point = kf.get_x();
                Eigen::Vector3d t_ = state_point.pos;
                Eigen::Matrix3d R_ = state_point.rot.toRotationMatrix();
                
                Eigen::JacobiSVD<Eigen::Matrix3d> svd_(R_, Eigen::ComputeFullU | Eigen::ComputeFullV);
                Eigen::Matrix3d R_b0bn_ = svd_.matrixU() * svd_.matrixV().transpose();
                Sophus::SE3d Tb0bn_;
                Tb0bn_ = Sophus::SE3d(R_b0bn_, t_);
                Sophus::SE3d Tc0b0_ = Tb0c0.inverse();
                Sophus::SE3d Tc0bn_ = Tc0b0_ * Tb0bn_;
                Sophus::SE3d Tc0cn_p_ = Tc0bn_ * Tb0c0;
                // Sophus::SE3d Tcw_p_ = Tc0cn_p_.inverse();
                // Sophus::SE3f Twc_p = Tcw_p_.inverse().cast<float>();
                poses_IESEKF.push_back(Tc0cn_p_.cast<float>());
                frameTimes_IESEKF.push_back(lidar_front);
                // std::cout how many poses are there in poses_IESEKF
                std::cout << "poses size = " << poses_IESEKF.size() << std::endl;
                
                if(!loopicpedonce && nLoopBuffer > 0 && loopchangedonce)
                {
                    Eigen::Vector3d t__ = state_point.pos;
                    Eigen::Matrix3d R__ = state_point.rot.toRotationMatrix();
                    Eigen::JacobiSVD<Eigen::Matrix3d> svd__(R__, Eigen::ComputeFullU | Eigen::ComputeFullV);
                    Eigen::Matrix3d R_b0bn__ = svd__.matrixU() * svd__.matrixV().transpose();
                    Sophus::SE3d Tiwi_;
                    Tiwi_ = Sophus::SE3d(R_b0bn__, t__);
                    // conver the double kf.getSum() to int 
                    double dSum = kf.getSum();
                    int dSum_int = dSum;
                    if(degenerate_index_mean != 0)
                    {
                        global_map.addPointCloudFrame(feats_undistort_G, lidar_front, tImLeft, dSum_int, Tiwi_, state_point, PointToAdd_global, PointNoNeedDownsample_global, PointToAdd_body_global, PointNoNeedDownsample_body_global);
                        global_map.buildsubMapTrajectory(lidar_front, Tc0cn_p_.cast<float>());
                    }
                    std::vector<std::pair<std::vector<double>, std::vector<Sophus::SE3f>>> submapTrajectories;
                    submapTrajectories = global_map.returnSubmapTrajectories();
                    std::vector<bool> submapDegenerated;
                    submapDegenerated = global_map.returnSubmapFeatureless();
                    std::pair<std::vector<double>, std::vector<Sophus::SE3f>> vslamtimeposes = SLAM.returnTrajectory();
                    std::pair<std::vector<double>, std::vector<Sophus::SE3f>> IESEKFposes = std::make_pair(frameTimes_IESEKF, poses_IESEKF);
                    // calculate SE3 transformation between vslamtimeposes and trajectories in submapTrajectories
                    for(int i=0; i< submapTrajectories.size(); i++)
                    {
                        if(!submapDegenerated[i])
                        {
                            ComputeSim3<double> computeSim3(0.04);
                            computeSim3.LoadTraj_fromVector(submapTrajectories[i], vslamtimeposes);
                            ...
                            Eigen::Matrix4d se3 = computeSim3.GetSE3();
                            Eigen::Matrix4d sim3 = computeSim3.GetSim3();
                            std::cout << "sim3 = " << sim3 << std::endl;
                            std::cout << "se3 = " << se3 << std::endl;
                            // convert se3 to Sophus::SE3d
                            Eigen::Matrix3d R_se3 = se3.block(0,0,3,3);
                            Eigen::Vector3d t_se3 = se3.block(0,3,3,1);
                            Eigen::JacobiSVD<Eigen::Matrix3d> svd_se3(R_se3, Eigen::ComputeFullU | Eigen::ComputeFullV);
                            Eigen::Matrix3d R_se3_orth = svd_se3.matrixU() * svd_se3.matrixV().transpose();
                            Sophus::SE3d Tse3;
                            Tse3 = Sophus::SE3d(R_se3_orth, t_se3);
                            std::cout << "Tse3 = " << Tse3.matrix() << std::endl;
                            std::pair<std::vector<double>, std::vector<Sophus::SE3f>> submapTrajectory_se3 = submapTrajectories[i];
                            // apply the transformation to the submap trajectory
                            for(int j = 0; j < submapTrajectories[i].first.size(); j++)
                            {
                                submapTrajectory_se3.second[j] = Tse3.cast<float>() * submapTrajectories[i].second[j];
                                // std::cout << "submapTrajectory_se3.second[j] = " << submapTrajectory_se3.second[j].matrix() << std::endl;

                            }
                            
                            std::string submapTrajectory = "/home/uas/submapTrajectory_SE3" + std::to_string(i) + ".txt";
                            std::ofstream submapTrajectoryFile;
                            submapTrajectoryFile.open(submapTrajectory);

                            for(int j = 0; j < submapTrajectory_se3.first.size(); j++)
                            {
                                Eigen::Vector3f twc = submapTrajectory_se3.second[j].translation();
                                Eigen::Quaternionf q = submapTrajectory_se3.second[j].unit_quaternion();

                                submapTrajectoryFile<< std::to_string(submapTrajectory_se3.first[j]) << " " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
                            }
                            submapTrajectoryFile.close();

                            std::vector<PointCloudFrame> submapPointClouds = global_map.returnSubMapPointCloudFrames(i);
                            std::cout << "submapPointClouds.size() = " << submapPointClouds.size() << std::endl;
                            std::pair<std::vector<double>, std::vector<Sophus::SE3f>> submapTrajectory_iwise3;
                            submapTrajectory_iwise3.first = submapTrajectory_se3.first;
                            std::cout << "submapTrajectory_iwise3.first.size() = " << submapTrajectory_iwise3.first.size() << std::endl;
                            // need to transform submapTrajectory_se3 to IESEKF coordinate system
                            for(int j = 0; j < submapTrajectory_se3.first.size(); j++)
                            {
                                Sophus::SE3f Twi_f;
                                Twi_f = submapTrajectory_se3.second[j] * Tleftb_f;
                                Sophus::SE3f Tiwi_f;

                                Tiwi_f = Tb0c0.cast<float>() * Twi_f;
                                submapTrajectory_iwise3.second.push_back(Tiwi_f);
                                
                                if(i==4)
                                {
                                    std::cout << "publishing old " << std::endl;
                                    // publish_frame_world_MS(laserCloudmsgs[j], pubLaserCloudFull_LOOP, states[j]);
                                    // need to transform the pointcloud in pcls using the Tse3 with Tbleft*Tse3*Tleftb 
                                    int size = submapPointClouds[j].getPoints()->size();
                                    for(int k=0; k<size; k++)
                                    {
                                        V3D p_old(submapPointClouds[j].getPoints()->points[k].x, submapPointClouds[j].getPoints()->points[k].y, submapPointClouds[j].getPoints()->points[k].z);
                                        V3D p_se3 = Tbleft*Tse3*Tleftb * p_old;
                                        submapPointClouds[j].getPoints()->points[k].x = p_se3(0);
                                        submapPointClouds[j].getPoints()->points[k].y = p_se3(1);
                                        submapPointClouds[j].getPoints()->points[k].z = p_se3(2);
                                    }
                                    sensor_msgs::PointCloud2 laserCloudmsg;
                                    pcl::toROSMsg(*submapPointClouds[j].getPoints(), laserCloudmsg);
                                    laserCloudmsg.header.stamp = ros::Time().fromSec(submapTrajectory_se3.first[j]);
                                    laserCloudmsg.header.frame_id = "camera_init";
                                    // set the decay time of the updated submap pointcloud to 100 seconds
                                    pubLaserCloudFull_LOOP.publish(laserCloudmsg);
                                    
                                }

                            }
                            std::string submapTrajectoryiwi = "/home/uas/submapTrajectory_SE3_iwi" + std::to_string(i) + ".txt";
                            std::ofstream submapTrajectoryFileiwi;
                            submapTrajectoryFileiwi.open(submapTrajectoryiwi);

                            for(int j = 0; j < submapTrajectory_iwise3.first.size(); j++)
                            {
                                Eigen::Vector3f twc = submapTrajectory_iwise3.second[j].translation();
                                Eigen::Quaternionf q = submapTrajectory_iwise3.second[j].unit_quaternion();

                                submapTrajectoryFileiwi<< std::to_string(submapTrajectory_iwise3.first[j]) << " " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
                            }
                            submapTrajectoryFileiwi.close();                            
                        }
                    }
                    // save each submap trajectory to a .txt file in the directory of /home/uas
                    for(int i = 0; i < submapTrajectories.size(); i++)
                    {
                        std::string submapTrajectory = "/home/uas/submapTrajectory" + std::to_string(i) + ".txt";
                        std::ofstream submapTrajectoryFile;
                        submapTrajectoryFile.open(submapTrajectory);
                        for(int j = 0; j < submapTrajectories[i].first.size(); j++)
                        {
                            Eigen::Vector3f twc = submapTrajectories[i].second[j].translation();
                            Eigen::Quaternionf q = submapTrajectories[i].second[j].unit_quaternion();

                            submapTrajectoryFile<< std::to_string(submapTrajectories[i].first[j]) << " " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
                        }

 
                        submapTrajectoryFile.close();
                    }
                    // need to update the submap trajectories using the loop closed VSLAM poses:vslamtimeposes, before changing current map
                    // starting from current submap, by default, the current submap is the last submap, starting from the last submap, get the submap trajectory, (whether calculate SE3 or Sim3, need to be tested, first SE3)
                    // if this submap is non-degenerated, calculate SE3 transformation using vslamtimeposes, apply SE3 transformation to the submap trajectory
                    // if this submap is degenerated, directly apply the vslamtimeposes to the submap trajectory by linearly interpolate using timestamps
                    // after transform the submap trajectory, publish the submap pointcloud using these transformed poses, the color of the updated submap pointcloud should be orange
                    // the decay time of the updated submap pointcloud should be 100 seconds
                    // after processing one submap, the submap id --, until the dMatchedTime falls into the time range of the submap, also do not update the matched submap
                    // after found the matched submap, change the current submap to the matched submap for further tracking
                    ...
                    std::ofstream loopclosedtrajetory;
                    loopclosedtrajetory.open("/home/uas/loopclosedtrajetory.txt");
                    for(int i = 0; i < vslamtimeposes.first.size(); i++)
                    {
                        Eigen::Vector3f twc = vslamtimeposes.second[i].translation();
                        Eigen::Quaternionf q = vslamtimeposes.second[i].unit_quaternion();

                        // loopclosedtrajetory << std::to_string(vslamtimepose.first[i]) << " " << vslamtimepose.second[i].translation().transpose() << std::endl;
                        loopclosedtrajetory<< std::to_string(vslamtimeposes.first[i]) << " " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;

                    }
                    loopclosedtrajetory.close();
                    std::cout << "loopclosedtrajetory.txt created" << std::endl;
                    std::ofstream loopclosedtrajetoryIESEKF;
                    loopclosedtrajetoryIESEKF.open("/home/uas/loopclosedtrajetoryIESEKF.txt");
                    std::cout << "frameTimes_IESEKF.size()"<< frameTimes_IESEKF.size()<< std::endl;
                    
                    for(int i = 0; i < frameTimes_IESEKF.size(); i++)
                    {
                        // loopclosedtrajetoryIESEKF << std::to_string(frameTimes_IESEKF[i]) << " " << poses_IESEKF[i].translation().transpose() << std::endl;
                        // // loopclosedtrajetoryIESEKF << std::to_string(frameTimes_IESEKF[i]) << std::endl;
                        // std::cout << "frameTimes_IESEKF[i] = " << std::to_string(frameTimes_IESEKF[i]) << std::endl;
                        Eigen::Vector3f twc = poses_IESEKF[i].translation().transpose();
                        Eigen::Quaternionf q = poses_IESEKF[i].unit_quaternion();
                        // loopclosedtrajetory << std::to_string(vslamtimepose.first[i]) << " " << vslamtimepose.second[i].translation().transpose() << std::endl;
                        loopclosedtrajetoryIESEKF<< std::to_string(frameTimes_IESEKF[i]) << " " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;

                    }
                    loopclosedtrajetoryIESEKF.close();
                    std::cout << "loopclosedtrajetoryIESEKF.txt created" << std::endl;
                ...
                times.push_back(lidar_front);
                states.push_back(state_point);
                // pcls.push_back(feats_undistort);
                ...
                if(degenerate_index_mean != 0)
                {
                    global_map.addPointCloudFrame(feats_undistort_G, lidar_front, tImLeft, dSum_int, Tiwi, state_point, PointToAdd_global, PointNoNeedDownsample_global, PointToAdd_body_global, PointNoNeedDownsample_body_global);
                    global_map.buildsubMapTrajectory(lidar_front, Tc0cn_p_.cast<float>());
                }
    ...
    std::pair<std::vector<double>, std::vector<Sophus::SE3f>> vslamtimepose = SLAM.returnTrajectory();
    // create a .txt file called loopclosedtrajetory.txt, for each row, output the time and the translation of the SE3f
    std::ofstream loopclosedtrajetory;
    loopclosedtrajetory.open("/home/uas/loopclosedtrajetorybe.txt");
    for(int i = 0; i < vslamtimepose.first.size(); i++)
    {
        Eigen::Vector3f twc = vslamtimepose.second[i].translation();
        Eigen::Quaternionf q = vslamtimepose.second[i].unit_quaternion();
        // loopclosedtrajetory << std::to_string(vslamtimepose.first[i]) << " " << vslamtimepose.second[i].translation().transpose() << std::endl;
        loopclosedtrajetory<< std::to_string(vslamtimepose.first[i]) << " " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    }
    loopclosedtrajetory.close();
    std::cout << "loopclosedtrajetory.txt created" << std::endl;
    std::ofstream loopclosedtrajetoryIESEKF;
    loopclosedtrajetoryIESEKF.open("/home/uas/loopclosedtrajetoryIESEKFbe.txt");
    std::cout << "frameTimes_IESEKF.size()"<< frameTimes_IESEKF.size()<< std::endl;
