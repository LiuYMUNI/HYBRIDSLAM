...
int main(int argc, char **argv)
{
                ...
                if(degenerate_index_mean<degenerate_index_mean_thres)
                {
                    if(!bLoopDetected)
                    {
                        if(nLoopBuffer > 0)
                        {
                            ...
                            rmse = 0.0f;
                            bRefine = refine_with_pt2pt_icp(feats_down_body, 0.11, 25, global_map.current_ikdtreeptr_, Ricp, ticp, state_point,rmse);
                            ...
                            kf.update_iterated_dyn_share_modified(LASER_POINT_COV, solve_H_time);
                            kf.change_x(state_point);
                            if(nLoopBuffer > nLoopBuffer_t - 4 )
                            {
                                kf.change_P(old_P);
                            }
                            ...
                            if(degenerate_index_mean<0.5*degenerate_index_mean_thres)
                            {
                                nLoopBuffer = nLoopBuffer + 20;
                            }
                            // if(rmse < 0.135)
                            // {
                            //     nLoopBuffer = nLoopBuffer - 5;
  ...
  SLAM.SaveKeyFrameTrajectoryTUM("/home/uas/MAA_KeyFrameTrajectory_TUM_Format.txt");
  SLAM.SaveTrajectoryTUM("/home/uas/MAA_FrameTrajectory_TUM_Format.txt");
