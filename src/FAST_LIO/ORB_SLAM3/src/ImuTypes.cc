...
Calib::Calib(const Calib &calib)
{
    ...
    mTbc = calib.mTbc;
    // // get the rotation matrix from the Sophus SE3 mTbc
    // Eigen::Matrix3f R = mTbc.so3().matrix();
    // // get the translation vector from the Sophus SE3 mTbc
    // Eigen::Vector3f t = mTbc.translation();
    // std::cout << "Calib::Calib(const Calib &calib) - R: " << R << std::endl;
    // std::cout << "Calib::Calib(const Calib &calib) - t: " << t << std::endl;
    mTcb = calib.mTcb;
    Cov = calib.Cov;
