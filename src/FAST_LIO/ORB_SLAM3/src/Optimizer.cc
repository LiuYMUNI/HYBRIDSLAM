...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    {
        Sophus::SE3<float> pose(pFrame->mRwb_p.cast<float>(),
                pFrame->mtwb_p.cast<float>());
        pFrame->SetPose(pose);
        std::cout << "FASTLIO POSE " << std::endl;
    }
...
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pKFi->bImu)
        {
            VertexVelocity* VV = new VertexVelocity(pKFi);
            VV->setId(maxKFid+3*(pKFi->mnId)+1);
            if(pKFi->mbneed_reoptimize &&  pKFi->mbprior_pose || !pKFi->mbprior_pose)
            {
                VV->setFixed(false);
            }
            else if(!pKFi->mbneed_reoptimize &&  pKFi->mbprior_pose)
            {
                VV->setFixed(true);
            }
    ...
    for(list<KeyFrame*>::iterator lit=lFixedKeyFrames.begin(), lend=lFixedKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        if(pKFi->bImu) // This should be done only for keyframe just before temporal window
        {
            VertexVelocity* VV = new VertexVelocity(pKFi);
            VV->setId(maxKFid+3*(pKFi->mnId)+1);
            if(pKFi->mbneed_reoptimize &&  pKFi->mbprior_pose)
            {
                VV->setFixed(false);
            }
            else
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame, bool bRecInit)
{
    ...
    if(pFrame->mbprior_pose)
    {
        ...
        auto trans_threshold = (0.06*Eigen::Vector3f::Ones()).norm();
        ...
        auto rot_threshold = (6.0/180.0*M_PI);
