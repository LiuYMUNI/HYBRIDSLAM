...
LoopClosing::LoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale, const bool bActiveLC):
    ...
    mbLoopDetected(false), mbLoopDetect(false), mdMatchedTime(0.0), mbMergeDetected(false), mnLoopNumNotFound(0), mnMergeNumNotFound(0), mbActiveLC(bActiveLC)
{
...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        // std::cout << "LoopClosing: 1" << std::endl;
...
#endif
            std::cout << "LoopClosing: 2" << std::endl;
            if(bFindedRegion)
            {
                if(mbMergeDetected)
                {
                    // std::cout << "LoopClosing: 3" << std::endl;
                    ...
                    if(mbLoopDetected)
                    {
                        ...
                        mbLoopDetect = mbLoopDetected;
                    }

                }
                std::cout << "LoopClosing: 4" << std::endl;
                if(mbLoopDetected)
                {   
                    std::cout << "LoopClosing: 5" << std::endl;
                    bool bGoodLoop = true;
                    vdPR_CurrentTime.push_back(mpCurrentKF->mTimeStamp);
                    vdPR_MatchedTime.push_back(mpLoopMatchedKF->mTimeStamp);
                    mdMatchedTime = mpLoopMatchedKF->mTimeStamp;
                    ...
                    if(mpCurrentKF->GetMap()->IsInertial())
                    {
                        ...
                        // if (fabs(phi(0))<0.02f && fabs(phi(1))<0.02f && fabs(phi(2))<0.349f) for yesterday good
                        // if (fabs(phi(0))<0.02f && fabs(phi(1))<0.02f && fabs(phi(2))<0.02) test not good
                        // (fabs(phi(0)) + fabs(phi(1)) + fabs(phi(2)))<0.06f not bad
                        if ((fabs(phi(0)) + fabs(phi(1)))<0.045f && fabs(phi(2)) < 0.16)
                        {
                            if(mpCurrentKF->GetMap()->IsInertial())
                            {
                                // If inertial, force only yaw
        ...
        // std::cout << "LoopClosing: 6" << std::endl;
        ResetIfRequested();

        if(CheckFinish()){
            break;
        }
        // std::cout << "LoopClosing: 7" << std::endl;
        usleep(5000);
    }
...
void LoopClosing::CorrectLoop()
{
...

    {
        ...
        for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
        {
            ...
            if(pKFi!=mpCurrentKF)
            {
                std::cout << "correcting KF: " << pKFi->mnId << std::endl;
