...
int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
{
    ...
    // make a copy of the pFrame
    Frame* pFrame_copy = new Frame(*pFrame);
    ...
    vector<EdgeMonoOnlyPose*> vpEdgesMono;
    vector<EdgeMonoOnlyPose*> vpEdgesMono_copy;
    vector<EdgeMonoOnlyPose*> vpEdgesMono_copy_new;
    vector<EdgeStereoOnlyPose*> vpEdgesStereo;
    vector<size_t> vnIndexEdgeMono;
    vector<size_t> vnIndexEdgeStereo;
    vpEdgesMono.reserve(N);
    vpEdgesMono_copy.reserve(N);
    vpEdgesMono_copy_new.reserve(N);
    vpEdgesStereo.reserve(N);
    vnIndexEdgeMono.reserve(N);
    vnIndexEdgeStereo.reserve(N);
    ...
    pFrame_copy->SetImuPoseVelocity(pFrame->mRwb_p.cast<float>(), pFrame->mtwb_p.cast<float>(), Eigen::Vector3f::Zero());
    VertexPose* VP_copy = new VertexPose(pFrame_copy);
    VP_copy->setId(8);
    VP_copy->setFixed(true);
    optimizer.addVertex(VP_copy);
    {
        unique_lock<mutex> lock(MapPoint::mGlobalMutex);
        ...
        for(int i=0; i<N; i++)
        {
            ...
            if(pMP)
            {
                ...
                if((!bRight && pFrame->mvuRight[i]<0) || i < Nleft)
                {
                    ...
                    EdgeMonoOnlyPose* e = new EdgeMonoOnlyPose(pMP->GetWorldPos(),0);
                    EdgeMonoOnlyPose* e_copy = new EdgeMonoOnlyPose(pMP->GetWorldPos(),0);
                    e->setVertex(0,VP);
                    e->setMeasurement(obs);

                    e_copy->setVertex(0,VP_copy);
                    e_copy->setMeasurement(obs);
                    // Add here uncerteinty
                    const float unc2 = pFrame->mpCamera->uncertainty2(obs);
                    // std::cout << "unc2 l: " << unc2 << std::endl;
                    const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave]/unc2;
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                    e_copy->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                    g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
                    e->setRobustKernel(rk);
                    rk->setDelta(thHuberMono);
                    ...
                    vpEdgesMono.push_back(e);
                    vpEdgesMono_copy.push_back(e_copy);
                    vnIndexEdgeMono.push_back(i);
                }
                // Stereo observation
                else if(!bRight)
    ...
    Frame* pFrame_copy_new = new Frame(*pFrame);
    // pFrame_copy_new->SetImuPoseVelocity(VP->estimate().Rwb.cast<float>(), VP->estimate().twb.cast<float>(), VV->estimate().cast<float>());

    VertexPose* VP_copy_new = new VertexPose(pFrame_copy_new);
    VP_copy_new->setId(9);
    VP_copy_new->setFixed(true);
    optimizer.addVertex(VP_copy_new);
    {
        unique_lock<mutex> lock(MapPoint::mGlobalMutex);
        ...
        for(int i=0; i<N; i++)
        {
            ...
            if(pMP)
            {
                ...
                if((!bRight && pFrame->mvuRight[i]<0) || i < Nleft)
                {
                    ...
                    // EdgeMonoOnlyPose* e = new EdgeMonoOnlyPose(pMP->GetWorldPos(),0);
                    EdgeMonoOnlyPose* e_copy_new = new EdgeMonoOnlyPose(pMP->GetWorldPos(),0);
                    // e->setVertex(0,VP);
                    // e->setMeasurement(obs);

                    e_copy_new->setVertex(0,VP_copy_new);
                    e_copy_new->setMeasurement(obs);
                    // Add here uncerteinty
                    const float unc2 = pFrame->mpCamera->uncertainty2(obs);
                    // std::cout << "unc2 l: " << unc2 << std::endl;
                    const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave]/unc2;
                    // e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                    e_copy_new->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                    // g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
                    // e->setRobustKernel(rk);
                    // rk->setDelta(thHuberMono);
                    ...
                    // vpEdgesMono.push_back(e);
                    vpEdgesMono_copy_new.push_back(e_copy_new);
                    // vnIndexEdgeMono.push_back(i);
                }
                // Stereo observation
                else if(!bRight)
                ...
                if(bRight && i >= Nleft)
                {
                    ...
                    // EdgeMonoOnlyPose* e = new EdgeMonoOnlyPose(pMP->GetWorldPos(),1);
                    EdgeMonoOnlyPose* e_copy_new = new EdgeMonoOnlyPose(pMP->GetWorldPos(),1);
                    // e->setVertex(0,VP);
                    // e->setMeasurement(obs);
