...
{
    // initialize the mTwg to be Sophus::SE3 with identity rotation and zero translation
    // mTwg = Sophus::SE3<float>();
    Eigen::Matrix3f R = Eigen::Matrix3f::Identity();
    Eigen::Vector3f t = Eigen::Vector3f::Zero();
    mTwg = Sophus::SE3<float>(R,t);
    // mbIniertialBA2 = false;
    mnMatchesInliers = 0;
...
// bool LocalMapping::Get_LM_IniertialBA2()
// {
//     std::cout << "Get_LM_IniertialBA2" << std::endl;
//     return mpCurrentKeyFrame->GetMap()->GetIniertialBA2();
// }
...
{
    std::cout << "Local Mapping started" << std::endl;
    mbFinished = false;

    while(1)
    {
        // std::cout << "Local Mapping running 1 " << std::endl;
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            // std::cout << "Local Mapping running 2 " << std::endl;
#ifdef REGISTER_TIMES
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
...
#endif
                // std::cout << "Local Mapping running 3 " << std::endl;
                // Initialize IMU here
                if(!mpCurrentKeyFrame->GetMap()->isImuInitialized() && mbInertial)
                {
                    ...
                    mpTracker->mRowg = mTwg.so3().matrix();
                    mpTracker->mtwg = mTwg.translation();
                    mpTracker->mbInertialBA0 = true;
                }
            ...
            // std::cout << "Local Mapping running 4 " << std::endl;
#ifdef REGISTER_TIMES
        ...
        // std::cout << "Local Mapping running 5 " << std::endl;
        ResetIfRequested();
    ...
    // std::cout << "Local Mapping running 6 " << std::endl;
    SetFinish();
}
...
void LocalMapping::KeyFrameCulling()
{
    ...
    // We only consider close stereo points redundant_th = 0.5;
    const int Nd = 21;
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    // Before this line we are not changing the map
    {
        ...
        if ((fabs(mScale - 1.f) > 0.00001) || !mbMonocular) {
            ...
            std::cout << "mTwg: " << mTwg.matrix() << std::endl;
            mTwg = Twg * mTwg;
            std::cout << "mTwg updated: " << mTwg.matrix() << std::endl;
            mpTracker->UpdateFrameIMU(mScale, vpKF[0]->GetImuBias(), mpCurrentKeyFrame);
            // log out the Twg
            std::cout << "Twg: " << Twg.matrix() << std::endl;
            std::cout << "coordinate updated" << std::endl;

            std::cout << "coordinate updated1" << std::endl;
        }
