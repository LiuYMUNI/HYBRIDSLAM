    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    // Fix verbosity VERBOSITY_DEBUG VERBOSITY_QUIET
...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    if(mpLoopCloser->mbLoopDetect)
    {
        mpTracker->mnLoopBuffer = 20;
    }
    Sophus::SE3f Tcw = mpTracker->GrabImageStereo(imLeftToFeed,imRightToFeed,timestamp,filename);

    // mTwg = mpTracker->mTwg;
    mRwg = mpTracker->mRowg;
    mtwg = mpTracker->mtwg;
    mbInertialBA0 = mpTracker->mbInertialBA0;
    mbInertialBA1 = mpTracker->mbInertialBA1;
    mbInertialBA2 = mpTracker->mbInertialBA2;
    mbLoopDetected = mpLoopCloser->mbLoopDetect;
    mdMatchedTime = mpLoopCloser->mdMatchedTime;
    mvl_WorldCoords = mpTracker->mCurrentFrame.mvl_WorldCoords;
    mvr_WorldCoords = mpTracker->mCurrentFrame.mvr_WorldCoords;
    mvl_observations = mpTracker->mCurrentFrame.mvl_observations;
    mvr_observations = mpTracker->mCurrentFrame.mvr_observations;
    mvl_invSigma2s = mpTracker->mCurrentFrame.mvl_invSigma2s;
    mvr_invSigma2s = mpTracker->mCurrentFrame.mvr_invSigma2s;
    // std::cout << "out grabber" << std::endl;

    unique_lock<mutex> lock2(mMutexState);
    mTrackingState = mpTracker->mState;
    std::cout << "mTrackingState: " << mTrackingState << std::endl;
    mtrackingStat = mpTracker->mState;
    std::cout << "mtrackingStat: " << mtrackingStat << std::endl;
...
Sophus::SE3f System::TrackStereo_Pose(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename, const Sophus::SE3d &Tc0bn, const Eigen::Matrix<double, 6, 6> &info_p, const Eigen::Vector3f &velocity_c0)
{
    if(mSensor!=STEREO && mSensor!=IMU_STEREO)
    {
        cerr << "ERROR: you called TrackStereo but input sensor was not set to Stereo nor Stereo-Inertial." << endl;
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageStereo_Pose(imLeftToFeed,imRightToFeed,timestamp, Tc0bn, info_p, velocity_c0, filename);
...
Sophus::SE3f System::TrackStereo_Pose_VB(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename, const Sophus::SE3d &Tc0bn, const Eigen::Matrix<double, 6, 6> &info_p, const Eigen::Vector3f &velocity_c0, const Eigen::Vector3d &ba, const Eigen::Vector3d &bg)
{
    if(mSensor!=STEREO && mSensor!=IMU_STEREO)
    {
        cerr << "ERROR: you called TrackStereo but input sensor was not set to Stereo nor Stereo-Inertial." << endl;
...
Sophus::SE3f System::TrackStereo_PCL(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename, pcl::PointCloud<pcl::PointXYZINormal>::Ptr pfeats_down_body)
{
    if(mSensor!=STEREO && mSensor!=IMU_STEREO)
    {
        cerr << "ERROR: you called TrackStereo but input sensor was not set to Stereo nor Stereo-Inertial." << endl;
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageStereo_PCL(imLeftToFeed,imRightToFeed,timestamp,filename,pfeats_down_body);
    int n = pfeats_down_body->points.size();
    std::cout << "point cloud size1: " << n << std::endl;
    // mTwg = mpTracker->mTwg;
    mRwg = mpTracker->mRowg;
    mtwg = mpTracker->mtwg;
    mbInertialBA0 = mpTracker->mbInertialBA0;
...
// Sophus::SE3f System::TrackStereo_PCL_PRIOR(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename, pcl::PointCloud<pcl::PointXYZINormal>::Ptr pfeats_down_body, const Sophus::SE3d &prior_wc)
// {
//     if(mSensor!=STEREO && mSensor!=IMU_STEREO)
//     {
//         cerr << "ERROR: you called TrackStereo but input sensor was not set to Stereo nor Stereo-Inertial." << endl;
...
//     Sophus::SE3f Tcw = mpTracker->GrabImageStereo(imLeftToFeed,imRightToFeed,timestamp,filename);
//     int n = pfeats_down_body->points.size();
//     std::cout << "point cloud size: " << n << std::endl;
//     // mTwg = mpTracker->mTwg;
//     mRwg = mpTracker->mRowg;
//     mtwg = mpTracker->mtwg;
//     mbInertialBA0 = mpTracker->mbInertialBA0;
...
// void System::SetIKdtree(KD_TREE<pcl::PointXYZINormal>* pIKdtree)
// {
//     // mIKdtree = IKdtree;
//     std::cout << "SetIKdtree success" << std::endl;
//     int ikdtree_size;
//     ikdtree_size = pIKdtree->size();
//     std::cout << "ikdtree_size in System: " << ikdtree_size << std::endl;
//     mpTracker->SetIKdtree(pIKdtree);
// }

void System::SetTleftb(const Sophus::SE3f &Tleftb)
{
    mTleftb = Tleftb;
    mTbleft = Tleftb.inverse();
    
    mpTracker->SetTleftb(Tleftb);
}
// void System::CheckIKdtree_size()
// {
//     std::cout << "CheckIKdtree_size in System" << std::endl;
//     mpTracker->CheckIKdtree_size();
// }
...
std::pair<std::vector<double>, std::vector<Sophus::SE3f>> System::returnTrajectory()
{
    if(mSensor == MONOCULAR)
    {
        cerr << "ERROR: returnTrajectory cannot be used for monocular." << endl;
        return std::make_pair(std::vector<double>(), std::vector<Sophus::SE3f>());
    }
    ...
    std::vector<Sophus::SE3f> framePoses;
        ...
        lend = mpTracker->mlRelativeFramePoses.end(); lit != lend; lit++, lRit++, lT++, lbL++)
    {
        ...
        frameTimes.push_back(*lT);
        framePoses.push_back(Twc);
    }

    return std::make_pair(frameTimes, framePoses);
}
