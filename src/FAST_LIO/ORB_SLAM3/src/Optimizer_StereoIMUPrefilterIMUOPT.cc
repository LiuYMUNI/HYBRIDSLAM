...
{
    // clear the pFrame->mvl_WorldCoords, pFrame->mvr_WorldCoords, pFrame->mvl_observations and pFrame->mvr_observations
    pFrame->mvl_WorldCoords.clear();
    pFrame->mvr_WorldCoords.clear();
    pFrame->mvl_observations.clear();
    pFrame->mvr_observations.clear();
    pFrame->mvl_invSigma2s.clear();
    pFrame->mvr_invSigma2s.clear();
    ...
    // std::cout << "POP6 " << std::endl;
    pFrame->SetPose(pose);

    // // log the error after optimization for debugging
    // for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
    // {
    //     ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose* e = vpEdgesMono[i];
    //     const size_t idx = vnIndexEdgeMono[i];
    ...
    //         // double error2 = error.dot(error);
    //         // const float chi2 = e->chi2();
    //         // std::cout << "visual left mode error: " << error2 << std::endl;
    //         // std::cout << "visual error: " << error[0] << std::endl;
    //         // std::cout << "visual error: " << error[1] << std::endl;
    //         // std::cout << "inertial mode chi2: " << chi2 << std::endl;
    //         // auto information = e->information();
    //         // std::cout << "inertial mode information: " << information << std::endl;
    //     }
    // }
    // for(size_t i=0, iend=vpEdgesMono_FHR.size(); i<iend; i++)
    // {
    ...
    //         // double error2 = error.dot(error);
    //         // const float chi2 = e->chi2();
    //         // std::cout << "visual right mode error: " << error2 << std::endl;
    //         // log the first dimension of the error vector
    //         // std::cout << "visual error: " << error[0] << std::endl;
    //         // std::cout << "visual error: " << error[1] << std::endl;
    //         // std::cout << "inertial mode chi2: " << chi2 << std::endl;
    //         // auto information = e->information();
    ...
    //     }
    // }
    // after optimization, need to save a copy of the inlier information of current frame
        ...
        if( !pFrame->mvbOutlier[i] )
        {
            ...
            if(pMP)
            {
                if(pFrame->mpCamera2)
                {
                    ...
                    {
                        kpUn = pFrame->mvKeys[i];
                        // push the kpUn.pt.x and kpUn.pt.y into the vector mvl_observations
                        Eigen::Matrix<double, 2, 1> obs;
                        obs << kpUn.pt.x, kpUn.pt.y;
                        pFrame->mvl_observations.push_back(obs);
                        const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];
                        // cast to double and push the invSigma2 twice into the vector mvl_InvLevelSigma2
                        pFrame->mvl_invSigma2s.push_back(invSigma2);
                        pFrame->mvl_invSigma2s.push_back(invSigma2);
                        
                        Eigen::Vector3d xw = pMP->GetWorldPos().cast<double>();
                        // push the 3D point into the vector mvl_WorldCoords
                        pFrame->mvl_WorldCoords.push_back(xw);
                    }
                    else
                    {
                        kpUn = pFrame->mvKeysRight[i - pFrame->Nleft];
                        ...
                        Eigen::Matrix<double, 2, 1> obs;
                        obs << kpUn.pt.x, kpUn.pt.y;
                        pFrame->mvr_observations.push_back(obs);
                        const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];
                        // cast to double and push the invSigma2 twice into the vector mvr_InvLevelSigma2
                        pFrame->mvr_invSigma2s.push_back(invSigma2);
                        pFrame->mvr_invSigma2s.push_back(invSigma2);
                        Eigen::Vector3d xw = pMP->GetWorldPos().cast<double>();
                        // push the 3D point into the vector mvr_WorldCoords
                        pFrame->mvr_WorldCoords.push_back(xw);
                    }
                }
            }
        }
    ...
    // std::cout << "POP7 " << std::endl;
    // std::cout << "should return to tracking" << std::endl;
...
{
    //std::cout << "LocalInertialBA" << std::endl;
    Map* pCurrentMap = pKF->GetMap();
    ...
    //std::cout << "LocalInertialBA0" << std::endl;
    ...
    //std::cout << "LocalInertialBA1" << std::endl;
    // Setup optimizer
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolverX::LinearSolverType * linearSolver;
    linearSolver = new g2o::LinearSolverEigen<g2o::BlockSolverX::PoseMatrixType>();
    ...
    //std::cout << "LocalInertialBA2" << std::endl;
    // Set Local temporal KeyFrame vertices
    N=vpOptimizableKFs.size();
    for(int i=0; i<N; i++)
    {
        // std:cout << "LocalInertialBA2.1" << std::endl;
        KeyFrame* pKFi = vpOptimizableKFs[i];
        //std::cout << "LocalInertialBA2.2" << std::endl;
        VertexPose * VP = new VertexPose(pKFi);
        //std::cout << "LocalInertialBA2.3" << std::endl;
        VP->setId(pKFi->mnId);
        // VP->setFixed(false);
        if(pKFi->mbprior_pose)
        {
            VP->setFixed(true);
        ...
        //std::cout << "LocalInertialBA2.4" << std::endl;
        optimizer.addVertex(VP);
    ...
    //std::cout << "LocalInertialBA3" << std::endl;
    // Set Fixed KeyFrame vertices
    ...
    //std::cout << "LocalInertialBA4" << std::endl;
    ...
    //std::cout << "LocalInertialBA5" << std::endl;
    // Set MapPoint vertices
    ...
    //std::cout << "LocalInertialBA6" << std::endl;
    ...
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        ...
        if(pMP->isBad())
        {
            std::cout << "Map point is bad" << std::endl;
            continue;
        }
    ...
    //std::cout << "LocalInertialBA7" << std::endl;
    // Stereo
    for(size_t i=0, iend=vpEdgesStereo.size(); i<iend;i++)
    {
    ...
    //std::cout << "LocalInertialBA8" << std::endl;
    ...
    if((2*err < err_end || isnan(err) || isnan(err_end)) && !bLarge) //bGN)
    {
        if(2*err < err_end)
        {
            std::cout << "2*err < err_end" << std::endl;
        }
        else if(isnan(err))
        {
            std::cout << "isnan(err)" << std::endl;
        }
        else if(isnan(err_end))
        {
            std::cout << "isnan(err_end)" << std::endl;
        }
        else if(!bLarge)
        {
            std::cout << "!bLarge" << std::endl;
        }
    ...
    if(!vToErase.empty())
    {
        std::cout << "Erasing " << vToErase.size() << " map points" << std::endl;
        for(size_t i=0;i<vToErase.size();i++)
        {
            KeyFrame* pKFi = vToErase[i].first;
            MapPoint* pMPi = vToErase[i].second;
    ...
    N=vpOptimizableKFs.size();
    //std::cout << "LocalInertialBA9" << std::endl;
    for(int i=0; i<N; i++)
    {
        KeyFrame* pKFi = vpOptimizableKFs[i];

        VertexPose* VP = static_cast<VertexPose*>(optimizer.vertex(pKFi->mnId));
        // std::cout << "Optimized posex: " << std::endl;
        Eigen::Matrix3f Rcw = VP->estimate().Rcw[0].cast<float>();
        // make sure it is orthogonal
        Eigen::JacobiSVD<Eigen::Matrix3f> svd(Rcw, Eigen::ComputeFullU | Eigen::ComputeFullV);
        Eigen::Matrix3f Rcw_orthogonal = svd.matrixU() * svd.matrixV().transpose();
        Sophus::SE3f Tcw(Rcw_orthogonal, VP->estimate().tcw[0].cast<float>());
        // std::cout << "Optimized posex 0: " << std::endl;
        pKFi->SetPose(Tcw);
    ...
    //std::cout << "LocalInertialBA10" << std::endl;
    ...
    for(list<KeyFrame*>::iterator it=lpOptVisKFs.begin(), itEnd = lpOptVisKFs.end(); it!=itEnd; it++)
    {
        ...
        // Sophus::SE3f Tcw(VP->estimate().Rcw[0].cast<float>(), VP->estimate().tcw[0].cast<float>());
        // Sophus::SE3f Tcw(Rcw_orthogonal, VP->estimate().tcw[0].cast<float>());
        // std::cout << "Optimized pose 1: " << std::endl;
        pKFi->SetPose(Tcw);
        pKFi->mnBALocalForKF=0;
    }
    //std::cout << "LocalInertialBA11" << std::endl;
    //Points
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        MapPoint* pMP = *lit;
    ...
    pMap->IncreaseChangeIndex();
    //std::cout << "LocalInertialBA12" << std::endl;
}
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame, bool bRecInit)
{
    if(!pFrame->mbprior_pose)
    {
        ...
        //     EdgePriorPoseLIO *ep = new EdgePriorPoseLIO(pFrame->mRwb_p, pFrame->mtwb_p);
        //     // std::cout << "pFrame->mRwb_p" << pFrame->mRwb_p << std::endl;
        //     // std::cout << "pFrame->mtwb_p" << pFrame->mtwb_p << std::endl;
        //     ep->setVertex(0, VP);
        //     ep->setInformation(pFrame->minfo_p);
        //     optimizer.addEdge(ep);
        //     std::cout << "prior pose edge added" << std::endl;
        // }
            ...
            for(int i=0; i<N; i++)
            {
                ...
                if(pMP)
                {
                    ...
                    if((!bRight && pFrame->mvuRight[i]<0) || i < Nleft)
                    {
                        ...
                        // std::cout << "unc2 l: " << unc2 << std::endl;
                        const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave]/unc2;
                        // std::cout << "invSigma2 l: " << invSigma2 << std::endl;
                        e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                    ...
                    if(bRight && i >= Nleft)
                    {
                        ...
                        // Add here uncerteinty
                        const float unc2 = pFrame->mpCamera->uncertainty2(obs);
                        // std::cout << "unc2 r: " << unc2 << std::endl;
                        const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave]/unc2;
                        // std::cout << "invSigma2 r: " << invSigma2 << std::endl;
                        e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
        ...
        // pFrame->SetImuPoseVelocity(VP->estimate().Rwb.cast<float>(), VP->estimate().twb.cast<float>(), VV->estimate().cast<float>());
        // std::cout<<"VP->estimate().Rwb.cast<float>()"<<VP->estimate().Rwb.cast<float>()<<std::endl;
        // std::cout<<"VP->estimate().twb.cast<float>()"<<VP->estimate().twb.cast<float>()<<std::endl;
        if(pFrame->mbprior_pose)
        {
            std::cout << "prior pose for point changing" << std::endl;
            Eigen::JacobiSVD<Eigen::Matrix3f> svd(VP->estimate().Rwb.cast<float>(), Eigen::ComputeFullU | Eigen::ComputeFullV);
            // std::cout << "svd.matrixU(): " << std::endl;
            Eigen::Matrix3f R_orthogonal = svd.matrixU() * svd.matrixV().transpose();
            // std::cout << "R_orthogonal: " << std::endl;
            Sophus::SE3f Twb = Sophus::SE3f(R_orthogonal, VP->estimate().twb.cast<float>());
            // std::cout << "Twb: " << std::endl;
            Sophus::SE3f Tbw = Twb.inverse();
            Eigen::JacobiSVD<Eigen::Matrix3f> svd1(pFrame->mRwb_p.cast<float>(), Eigen::ComputeFullU | Eigen::ComputeFullV);
            // std::cout << "svd1.matrixU(): " << std::endl;
            Eigen::Matrix3f R_orthogonal1 = svd1.matrixU() * svd1.matrixV().transpose();
            // std::cout << "R_orthogonal1: " << std::endl;
            Sophus::SE3f Twb_p = Sophus::SE3f(R_orthogonal1, pFrame->mtwb_p.cast<float>());
            // std::cout << "VP->estimate().Rwb: " << VP->estimate().Rwb << std::endl;
            // std::cout << "VP->estimate().twb: " << VP->estimate().twb << std::endl;
            // {
            //     unique_lock<mutex> lock(MapPoint::mGlobalMutex);
            // compare the Twb_p.translation() and Twb.translation(), calculate the difference, if the length is larger than 0.1, then update the point
            auto trans_diff = (Twb_p.translation() - Twb.translation()).norm();
            auto trans_threshold = (0.15*Eigen::Vector3f::Ones()).norm();
            bool update_point_t = false;
            bool update_point_r = false;
            if(trans_diff > trans_threshold)
            {
                update_point_t = true;
            }
            // calculate the rotation difference, convert to euler angle in gradian, if the angle is larger than 10 degree, then update the point
            auto rot_diff = (Twb_p.so3().inverse() * Twb.so3()).log().norm();
            auto rot_threshold = (15.0/180.0*M_PI);
            ...
            // {
            //     unique_lock<mutex> lock(MapPoint::mGlobalMutex);
            // std::cout << "pFrame->mvpMapPoints.size(): " << pFrame->mvpMapPoints.size() << std::endl;
            if(true)
            {
                // for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
                // {
                ...
                //         if(!pFrame->mvbOutlier[idx])
                //         {
                //             // std::cout << "pMP->GetWorldPos(): " << pMP->GetWorldPos() << std::endl;
                //             Eigen::Vector3f p_wp= pMP->GetWorldPos();
                //             // std::cout << "p_wp: " << p_wp << std::endl;
                //             Eigen::Vector3f p_bp = Tbw*p_wp;
                //             // std::cout << "p_bp: " << p_bp << std::endl;
                //             Eigen::Vector3f p_wpp = Twb_p.cast<float>()*p_bp;
                //             // std::cout << "p_wpp: " << p_wpp << std::endl;
                //             // std::cout <<"Twb: "<<Twb.matrix()<<std::endl;
                //             // std::cout <<"Twb_p: "<<Twb_p.matrix()<<std::endl;
                //             pMP->SetWorldPos(p_wpp);
                //             pMP->UpdateNormalAndDepth();
                //         }
                //         // else
                //         // {
                ...
                //         //     Eigen::Vector3f p_bp = Tbw*p_wp;
                //         //     Eigen::Vector3f p_wpp = Twb_p.cast<float>()*p_bp;
                //         //     Eigen::Vector3f p_w_mean = (p_wpp + p_wp)/2;
                //         //     pMP->SetWorldPos(p_wpp);
                //         //     pMP->UpdateNormalAndDepth();
                //         // }
                //         // std::cout << "pMP->GetWorldPos(): " << pMP->GetWorldPos() << std::endl;
                ...
                //         // Eigen::Vector3f p_bp = Tbw*p_wp;
                //         // std::cout << "p_bp: " << p_bp << std::endl;
                //         // Eigen::Vector3f p_wpp = pFrame->mTwb_p.cast<float>()*p_bp;
                //         // std::cout << "p_wpp: " << p_wpp << std::endl;
                //         // pMP->SetWorldPos(p_wpp);
                ...
                // pMap->IncreaseChangeIndex();
                // }
                // std::cout<<"pFrame->mTwb_p.cast<float>()" <<pFrame->mTTwb_p.cast<float>().matrix()<< std::endl;
                double current_t = pFrame->mTimeStamp;
                double last_t = pFrame->mpPrevFrame->mTimeStamp;
                double delta_t = current_t - last_t;
                Eigen::Vector3f v;
                //delta_t > 0
                if(false)
                {
                    Eigen::Matrix<float,3,1> last_position = pFrame->mpPrevFrame->GetImuPosition();
                    // convert the matrix to vector3f
                    Eigen::Vector3f last_position_f;
                    last_position_f << last_position(0), last_position(1), last_position(2);
                    v = (pFrame->mtwb_p.cast<float>() - last_position_f)/delta_t;
                    // std::cout << "calculated v: " << v << std::endl;
                    // pMP->SetVelocity(v);
                    pFrame->SetImuPoseVelocity(pFrame->mRwb_p.cast<float>(), pFrame->mtwb_p.cast<float>(), v);
                }
                else
                {
                    pFrame->SetImuPoseVelocity(pFrame->mRwb_p.cast<float>(), pFrame->mtwb_p.cast<float>(), VV->estimate().cast<float>());
                }
            ...
            {
                // std::cout << "small difference, no change point" << std::endl;
                pFrame->SetImuPoseVelocity(VP->estimate().Rwb.cast<float>(), VP->estimate().twb.cast<float>(), VV->estimate().cast<float>());
            }
        }
        else
        {
            // std::cout << "prior pose for point not changing" << std::endl;
            pFrame->SetImuPoseVelocity(VP->estimate().Rwb.cast<float>(), VP->estimate().twb.cast<float>(), VV->estimate().cast<float>());
        }
        Vector6d b;
        b << VG->estimate(), VA->estimate();
    ...
    else
    {
        ...
        // restart the optimization to correct IMU bias and velocity

        g2o::SparseOptimizer optimizer_re;
        g2o::BlockSolverX::LinearSolverType * linearSolver_re;

        linearSolver_re = new g2o::LinearSolverDense<g2o::BlockSolverX::PoseMatrixType>();

        g2o::BlockSolverX * solver_ptr_re = new g2o::BlockSolverX(linearSolver_re);

        g2o::OptimizationAlgorithmGaussNewton* solver_re = new g2o::OptimizationAlgorithmGaussNewton(solver_ptr_re);
        optimizer_re.setAlgorithm(solver_re);
        optimizer_re.setVerbose(false);


        // Set Current Frame vertex
        VertexPose* VP_re = new VertexPose(pFrame);
        VP_re->setId(0);
        VP_re->setFixed(true);
        optimizer_re.addVertex(VP_re);
        VertexVelocity* VV_re = new VertexVelocity(pFrame);
        VV_re->setId(1);
        VV_re->setFixed(false);
        optimizer_re.addVertex(VV_re);
        VertexGyroBias* VG_re = new VertexGyroBias(pFrame);
        VG_re->setId(2);
        VG_re->setFixed(false);
        optimizer_re.addVertex(VG_re);
        VertexAccBias* VA_re = new VertexAccBias(pFrame);
        VA_re->setId(3);
        VA_re->setFixed(false);
        optimizer_re.addVertex(VA_re);

        KeyFrame* pKF_re = pFrame->mpLastKeyFrame;
        VertexPose* VPk_re = new VertexPose(pKF_re);
        VPk_re->setId(4);
        VPk_re->setFixed(true);
        optimizer_re.addVertex(VPk_re);
        VertexVelocity* VVk_re = new VertexVelocity(pKF_re);
        VVk_re->setId(5);
        VVk_re->setFixed(true);
        optimizer_re.addVertex(VVk_re);
        VertexGyroBias* VGk_re = new VertexGyroBias(pKF_re);
        VGk_re->setId(6);
        VGk_re->setFixed(true);
        optimizer_re.addVertex(VGk_re);
        VertexAccBias* VAk_re = new VertexAccBias(pKF_re);
        VAk_re->setId(7);
        VAk_re->setFixed(true);
        optimizer_re.addVertex(VAk_re);

        EdgeInertial* ei_re = new EdgeInertial(pFrame->mpImuPreintegrated);

        ei_re->setVertex(0, VPk_re);
        ei_re->setVertex(1, VVk_re);
        ei_re->setVertex(2, VGk_re);
        ei_re->setVertex(3, VAk_re);
        ei_re->setVertex(4, VP_re);
        ei_re->setVertex(5, VV_re);
        optimizer_re.addEdge(ei_re);

        EdgeGyroRW* egr_re = new EdgeGyroRW();
        egr_re->setVertex(0,VGk_re);
        egr_re->setVertex(1,VG_re);
        Eigen::Matrix3d InfoG_re = pFrame->mpImuPreintegrated->C.block<3,3>(9,9).cast<double>().inverse();
        egr_re->setInformation(InfoG_re);
        optimizer_re.addEdge(egr_re);

        EdgeAccRW* ear_re = new EdgeAccRW();
        ear_re->setVertex(0,VAk_re);
        ear_re->setVertex(1,VA_re);
        Eigen::Matrix3d InfoA_re = pFrame->mpImuPreintegrated->C.block<3,3>(12,12).cast<double>().inverse();
        ear_re->setInformation(InfoA_re);
        optimizer_re.addEdge(ear_re);
        for(size_t it=0; it<4; it++)
        {
            optimizer_re.initializeOptimization(0);
            optimizer_re.optimize(10);
        }
        pFrame->SetImuPoseVelocity(pFrame->mRwb_p.cast<float>(), pFrame->mtwb_p.cast<float>(), VV_re->estimate().cast<float>());

        Vector6d b;
        b << VG_re->estimate(), VA_re->estimate();
        ...
        H.block<9,9>(0,0)+= ei_re->GetHessian2();
        H.block<3,3>(9,9) += egr_re->GetHessian2();
        H.block<3,3>(12,12) += ear_re->GetHessian2();
        std::cout << "before get hessian " << std::endl;
        int tot_in = 0, tot_out = 0;
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            EdgeMonoOnlyPose* e = vpEdgesMono[i];
        ...
        std::cout << "after get hessian " << std::endl;
        for(size_t i=0, iend=vpEdgesStereo.size(); i<iend; i++)
        {
            EdgeStereoOnlyPose* e = vpEdgesStereo[i];
        ...
        pFrame->mpcpi = new ConstraintPoseImu(VP_re->estimate().Rwb,VP_re->estimate().twb,VV_re->estimate(),VG_re->estimate(),VA_re->estimate(),H);
...
// int Optimizer::PoseInertialOptimizationLastKeyFrame_PCL(Frame *pFrame, KD_TREE<pcl::PointXYZINormal>* pIKdtree, pcl::PointCloud<pcl::PointXYZINormal>::Ptr pfeats_down_body, Eigen::Matrix3d Ryc0, bool bRecInit)
// {
//     g2o::SparseOptimizer optimizer;
//     g2o::BlockSolverX::LinearSolverType * linearSolver;
...
//     const Eigen::Matrix3d &Rbc = VP->estimate().Rbc[0];
//     const Eigen::Vector3d &tbc = VP->estimate().tbc[0];
...
//     int cu_pcl_size;
//     cu_pcl_size = pfeats_down_body->points.size();
//     int n1;
//     n1 = pIKdtree->size();
//     // std::cout << "ikdtree size inside optimizer: " << n1 << std::endl;


//     // std::cout << "pFrame->mpfeats_down_body size inside optimizer: " << cu_pcl_size << std::endl;
//     Eigen::Matrix3d Rc0y = Ryc0.inverse();
//     // std::cout << "1 " << std::endl;
//     Eigen::Vector3d t_I_L;
//     int coutcount;
//     coutcount = 0;
//     // std::cout << "2 " << std::endl;
//     vector<EdgeLiDARPlane*> vpEdgesLiDAR;

//     for(int i = 0; i < cu_pcl_size; i++)
//     {
//         // std::cout << "2.1" << std::endl;
//         pcl::PointXYZINormal &p = pfeats_down_body->points[i];
//         Eigen::Vector3d p_lidar(p.x, p.y, p.z);
//         // std::cout << "2.2" << std::endl;
//         t_I_L<<-0.011, -0.02329, 0.04412;
//         Eigen::Vector3d p_body;
//         p_body = p_lidar - t_I_L;
//         Eigen::Vector3d p_global_y;
//         p_global_y = VP->estimate().Rwb*p_body + VP->estimate().twb;
//         // std::cout << "2.3" << std::endl;
//         Eigen::Vector3d p_global_c0;
//         p_global_c0 = Rc0y*p_global_y;
//         Eigen::Vector3d p_global_i0;
//         p_global_i0 = Rbc*p_global_c0 + tbc;
//         pcl::PointXYZINormal point_world;
//         // std::cout << "2.4" << std::endl;
//         point_world.x = p_global_i0(0);
//         point_world.y = p_global_i0(1);
//         point_world.z = p_global_i0(2);
//         point_world.intensity = p.intensity;
//         // std::cout << "2.5" << std::endl;
//         std::vector<float> pointSearchSqDis(5);
//         std::vector<pcl::PointXYZINormal, Eigen::aligned_allocator<pcl::PointXYZINormal>> points_near;
//         pIKdtree->Nearest_Search(point_world, 5, points_near, pointSearchSqDis);
//         // std::cout << "2.6" << std::endl;
//         bool point_selected_surf;
//         point_selected_surf = points_near.size() < 5 ? false : pointSearchSqDis[5 - 1] > 5 ? false : true;
//         EdgeLiDARPlane* e = new EdgeLiDARPlane(p_body, p_lidar, Rc0y);
//         // std::cout << "2.7" << std::endl;
//         if (!point_selected_surf)
//         {
//             // std::cout << "2.8" << std::endl;
//             e->setVertex(0, VP);
//             e->setInformation(Eigen::Matrix<double, 1, 1>::Constant(1.0 / (0.001)));
//             g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
//             e->setRobustKernel(rk);
//             e->setLevel(0);
//             rk->setDelta(0.1);
...
//             normal<<pabcd(0), pabcd(1), pabcd(2);
...
//         for(size_t i=0, iend=vpEdgesLiDAR.size(); i<iend; i++)
//         {
//             EdgeLiDARPlane* e = vpEdgesLiDAR[i];
//             pcl::PointXYZINormal &p = pfeats_down_body->points[i];
//             Eigen::Vector3d p_body = e->p_body;
//             Eigen::Vector3d p_lidar = e->p_lidar;
//             Eigen::Vector3d p_global_y;
//             p_global_y = VP->estimate().Rwb*p_body + VP->estimate().twb;
//             Eigen::Vector3d p_global_c0;
//             p_global_c0 = Rc0y*p_global_y;
...
//                 if(s > 0.9)
//                 {
//                     // update the plane and the normal vector
//                     e->pabcd = pabcd;
//                     e->normal = normal;
//                     e->setLevel(0);
//                     if(it==2)
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
{
    if(!pFrame->mbprior_pose)
    {
            ...
            for(int i=0; i<N; i++)
            {
                ...
                if(pMP)
                {
                    ...
                    if(bRight && i >= Nleft)
                    {
                        ...
                        EdgeMonoOnlyPose* e = new EdgeMonoOnlyPose(pMP->GetWorldPos(),1);
                        EdgeMonoOnlyPose* e_copy = new EdgeMonoOnlyPose(pMP->GetWorldPos(),1);
                        e->setVertex(0,VP);
                        e->setMeasurement(obs);
        ...
        if(pFrame->mbprior_pose)
        {
            ...
            // compare the Twb_p.translation() and Twb.translation(), calculate the difference, if the length is larger than 0.1, then update the point
            auto trans_diff = (Twb_p.translation() - Twb.translation()).norm();
            // log out the trans_diff
            std::cout << "trans_diff" << trans_diff << std::endl;
            auto trans_threshold = (0.1*Eigen::Vector3f::Ones()).norm();
            std::cout << "trans_threshold" << trans_threshold << std::endl;
            bool update_point_t = false;
            bool update_point_r = false;
            if(trans_diff > 0.15)
            {
            ...
            if(true)
            {
                if(false)
                {
                    std::cout << "large difference, change point" << std::endl;
                    for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
                    {
                        const size_t idx = vnIndexEdgeMono[i];
                        // std::cout << "i: " << i << std::endl;
        ...
        // // log out the error information after optimization
        double error_dotsum = 0;
        double error_dotsum_copy = 0;
        double error_dotsum_copy_new = 0;
        // for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        // {
        //     EdgeMonoOnlyPose* e = vpEdgesMono[i];
        //     const size_t idx = vnIndexEdgeMono[i];
        ...
        //     auto error = e->error();
        //     // e->linearizeOplus();
        //     // auto _jacobianOplusXi = e->jacobianOplusXi();
        //     // std::cout << "_jacobianOplusXi: " << _jacobianOplusXi << std::endl;
        //     // std::cout << "i: " << i << std::endl;
        //     // std::cout << "inertial error: " << error[0] << std::endl;
        //     // std::cout << "inertial error: " << error[1] << std::endl;
        //     // sum of squared error
        //     double error2 = error.dot(error);
        //     error_dotsum += error2;
        //     EdgeMonoOnlyPose* e_copy = vpEdgesMono_copy[i];
        //     e_copy->computeError();
        //     auto error_copy = e_copy->error();
        //     double error2_copy = error_copy.dot(error_copy);
        //     error_dotsum_copy += error2_copy;
        //     EdgeMonoOnlyPose* e_copy_new = vpEdgesMono_copy_new[i];
        //     e_copy_new->computeError();
        //     auto error_copy_new = e_copy_new->error();
        //     double error2_copy_new = error_copy_new.dot(error_copy_new);
        //     error_dotsum_copy_new += error2_copy_new;
        //     // if(!pFrame->mvbOutlier[idx])
        //     // {
        //     //     e->computeError();
        //     //     auto error = e->error();
        ...
        //     //     error_dotsum += error2;
        //     //     // const float chi2 = e->chi2();
        //     //     // std::cout << "inertial mode error: " << error2 << std::endl;
        //     //     // std::cout << "inertial mode chi2: " << chi2 << std::endl;
        //     //     // auto information = e->information();
        //     //     // std::cout << "inertial mode information: " << information << std::endl;
        //     // }
        // }
        // std::cout << "error_dotsum: " << error_dotsum << std::endl;
        // std::cout << "error_dotsum_copy: " << error_dotsum_copy << std::endl;
        // std::cout << "error_dotsum_copy_new: " << error_dotsum_copy_new << std::endl;
    ...
    else
    {
        ...
        if(true)
        {
            g2o::SparseOptimizer optimizer_filter;
            g2o::BlockSolverX::LinearSolverType * linearSolver_filter;
            linearSolver_filter = new g2o::LinearSolverDense<g2o::BlockSolverX::PoseMatrixType>();
            g2o::BlockSolverX * solver_ptr_filter = new g2o::BlockSolverX(linearSolver_filter);
            g2o::OptimizationAlgorithmGaussNewton* solver = new g2o::OptimizationAlgorithmGaussNewton(solver_ptr_filter);
            optimizer_filter.setAlgorithm(solver);
            optimizer_filter.setVerbose(false);

            VertexVelocity* VV_filter = new VertexVelocity(pFrame);
            VV_filter->setId(1);
            VV_filter->setFixed(false);
            optimizer_filter.addVertex(VV_filter);
            pFrame->SetImuPoseVelocity(pFrame->mRwb_p.cast<float>(), pFrame->mtwb_p.cast<float>(), VV_filter->estimate().cast<float>());
            // Set Current Frame vertex
            VertexPose* VP_filter = new VertexPose(pFrame);
            VP_filter->setId(0);
            VP_filter->setFixed(true);
            optimizer_filter.addVertex(VP_filter);

            // Set MapPoint vertices
            const int N_filter = pFrame->N;
            const int Nleft_filter = pFrame->Nleft;
            const bool bRight_filter = (Nleft_filter!=-1);

            vector<EdgeMonoOnlyPose*> vpEdgesMono_filter;
            vector<EdgeStereoOnlyPose*> vpEdgesStereo_filter;
            vector<size_t> vnIndexEdgeMono_filter;
            vector<size_t> vnIndexEdgeStereo_filter;
            vpEdgesMono_filter.reserve(N_filter);
            vpEdgesStereo_filter.reserve(N_filter);
            vnIndexEdgeMono_filter.reserve(N_filter);
            vnIndexEdgeStereo_filter.reserve(N_filter);

            const float thHuberMono = sqrt(5.991);
            const float thHuberStereo = sqrt(7.815);

            const float chi2Mono_filter = 5.991;
            float chi2close_filter = 1.5*chi2Mono_filter;
            {
                unique_lock<mutex> lock(MapPoint::mGlobalMutex);
                ...
                for(int i=0; i<N_filter; i++)
                {
                    ...
                    if(pMP)
                    {
                        cv::KeyPoint kpUn;
                        // Left monocular observation
                        if((!bRight_filter && pFrame->mvuRight[i]<0) || i < Nleft_filter)
                        {
                            nInitialCorrespondences ++;
                            if(i < Nleft_filter) // pair left-right
                                kpUn = pFrame->mvKeys[i];
                            else
                                kpUn = pFrame->mvKeysUn[i];
                            ...
                            optimizer_filter.addEdge(e);
                            e->computeError();
                            bool bClose = pFrame->mvpMapPoints[i]->mTrackDepth<10.f;
                            const float chi2 = e->chi2();
                            if((chi2>chi2Mono_filter&&!bClose)||(bClose && chi2>chi2close_filter)||!e->isDepthPositive())
                            {
                            ...
                            vpEdgesMono_filter.push_back(e);
                            vnIndexEdgeMono_filter.push_back(i);

                        }
                        // Stereo observation
                        // Right monocular observation
                        if(bRight_filter && i >= Nleft_filter)
                        {
                            nInitialCorrespondences ++;
                            // nInitialMonoCorrespondences++;
                            pFrame->mvbOutlier[i] = false;

                            kpUn = pFrame->mvKeysRight[i - Nleft_filter];
                            Eigen::Matrix<double,2,1> obs;
                            obs << kpUn.pt.x, kpUn.pt.y;
                ...
                if ((nInliers<30) && !bRecInit)
                {
                    ...
                    for(size_t i=0, iend=vnIndexEdgeMono_filter.size(); i<iend; i++)
                    {
                        const size_t idx = vnIndexEdgeMono_filter[i];
                        e1 = vpEdgesMono_filter[i];
                        e1->computeError();
                        if (e1->chi2()<chi2MonoOut)
                            pFrame->mvbOutlier[idx]=false;
                        else
            ...
            std::cout << "inliers filtered" << std::endl;
        }
...
// int Optimizer::PoseInertialOptimizationLastFrame_PCL(Frame *pFrame, KD_TREE<pcl::PointXYZINormal>* pIKdtree, pcl::PointCloud<pcl::PointXYZINormal>::Ptr pfeats_down_body, Eigen::Matrix3d Ryc0, bool bRecInit)
// {
//     g2o::SparseOptimizer optimizer;
//     g2o::BlockSolverX::LinearSolverType * linearSolver;
...
//     // std::cout << "3 " << std::endl;
//     // for each lidar point in the pfeats_down_body, find their nearst points, and estimate the plane, and calculate the distance
...
//     const float chi2Stereo[8]={15.6f,9.8f,7.815f,7.815f,7.815f,7.815f,7.815f,7.815f};
//     const int its[8]={10,10,10,10,10,10,10,10};
...
bool Optimizer::esti_plane(Eigen::Matrix<float, 4, 1> &pca_result, const std::vector<pcl::PointXYZINormal, Eigen::aligned_allocator<pcl::PointXYZINormal>> &point, const float &threshold)
{
