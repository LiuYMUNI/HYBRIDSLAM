...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame, bool bRecInit)
{
    ...
    if(pFrame->mbprior_pose)
    {
        ...
        auto rot_threshold = (8.0/180.0*M_PI);
        if(rot_diff > rot_threshold)
        {
            update_point_r = true;
        }
        if(update_point_t || update_point_r)
        {
            pFrame->mbneed_reoptimize = true;
        }
        else
        {
            pFrame->mbneed_reoptimize = false;
        }
        // {
        //     unique_lock<mutex> lock(MapPoint::mGlobalMutex);
    ...
    if(pFrame->mbprior_pose)
    {
        pFrame->mpcpi = new ConstraintPoseImu(pFrame->mRwb_p, pFrame->mtwb_p,VV->estimate(),VG->estimate(),VA->estimate(),H);
    }
    else
    {
...
{
    // using prior pose to filter inliers
    if(pFrame->mbprior_pose)
    {
    ...
    std::cout << "error_dotsum before change: " << error_dotsum << std::endl;
    // Recover optimized pose, velocity and biases
    // pFrame->SetImuPoseVelocity(VP->estimate().Rwb.cast<float>(), VP->estimate().twb.cast<float>(), VV->estimate().cast<float>());
    // std::cout<<"VP->estimate().Rwb.cast<float>()"<<VP->estimate().Rwb.cast<float>()<<std::endl;
    // std::cout<<"VP->estimate().twb.cast<float>()"<<VP->estimate().twb.cast<float>()<<std::endl;
    if(pFrame->mbprior_pose)
    {
        ...
        if(true)
        {
            ...
            if(true)
            {
                ...
                v = (pFrame->mtwb_p.cast<float>() - last_position_f)/delta_t;
                // std::cout << "calculated v: " << v << std::endl;
                // pMP->SetVelocity(v); pFrame->mvelocity_y_bn.cast<float>()
                pFrame->SetImuPoseVelocity(pFrame->mRwb_p.cast<float>(), pFrame->mtwb_p.cast<float>(), VV->estimate().cast<float>());
            }
            else
            {
    ...
    if(pFrame->mbprior_pose)
    {
        VP->setEstimate(ImuCamPose(pFrame));
        double error_dotsum_c= 0;
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            if(!pFrame->mvbOutlier[idx])
            {
                ...
                auto error = e->error();
                double error2 = error.dot(error);
                error_dotsum_c += error2;

            }
        }
        std::cout << "error_dotsum after change: " << error_dotsum_c << std::endl;
    }
    ...
    if(pFrame->mbprior_pose)
    {
        pFrame->mpcpi = new ConstraintPoseImu(pFrame->mRwb_p, pFrame->mtwb_p,VV->estimate(),VG->estimate(),VA->estimate(),H.block<15,15>(15,15));
    }
    else
    {
