...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    mRowg = Eigen::Matrix3f::Identity();
    mtwg = Eigen::Vector3f::Zero();
    ...
    // get the rotation of Se3f 
    Eigen::Matrix3f Rbc = Tbc.so3().matrix();
    // get the translation of Se3f
    Eigen::Vector3f tbc = Tbc.translation();
    ...
    if(mpImuPreintegratedFromLastKF){
        std::cout << "IMU preintegration from last KF created from loader" << std::endl;
    }
}
...
bool Tracking::ParseIMUParamFile(cv::FileStorage &fSettings)
{
    ...
    mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(),*mpImuCalib);
    if(mpImuPreintegratedFromLastKF){
        std::cout << "IMU preintegration from last KF created from file" << std::endl;
    }
...
// Sophus::SE3f Tracking::GrabImageStereo_PCL(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename, PointCloudXYZI::Ptr pfeats_down_body)
// {
...
// #endif
//     mCurrentFrame.SetPCL(pfeats_down_body);
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    mCurrentFrame.mNameFile = filename;
    mCurrentFrame.mnDataset = mnNumDataset;
    mCurrentFrame.mpfeats_down_body = nullptr;
    mCurrentFrame.mbprior_pose = false;
    mCurrentFrame.stereo_setflag = false;
    mCurrentFrame.mbneed_reoptimize = false;
...
Sophus::SE3f Tracking::GrabImageStereo_Pose(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, const Sophus::SE3d &Tc0bn, const Eigen::Matrix<double, 6, 6> &info, const Eigen::Vector3f &velocity_c0, string filename)
{
    //cout << "GrabImageStereo" << endl;
    ...
    mCurrentFrame.mnDataset = mnNumDataset;
    mCurrentFrame.mpfeats_down_body = nullptr;
    mCurrentFrame.mbprior_pose = true;
    mCurrentFrame.stereo_setflag = false;
    mCurrentFrame.mTc0bn = Tc0bn;
    mCurrentFrame.minfo_p = info;
    mCurrentFrame.mvelocity_c0 = velocity_c0;
...
Sophus::SE3f Tracking::GrabImageStereo_Pose_VB(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, const Sophus::SE3d &Tc0bn, const Eigen::Matrix<double, 6, 6> &info, const Eigen::Vector3f &velocity_c0, const Eigen::Vector3d &ba, const Eigen::Vector3d &bg, string filename)
{
    //cout << "GrabImageStereo" << endl;
    ...
    mCurrentFrame.minfo_p = info;
    mCurrentFrame.mvelocity_c0 = velocity_c0;
    mCurrentFrame.mba_p = ba;
    mCurrentFrame.mbg_p = bg;
...
Sophus::SE3f Tracking::GrabImageStereo_PCL(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename, pcl::PointCloud<pcl::PointXYZINormal>::Ptr pfeats_down_body)
{
    //cout << "GrabImageStereo" << endl;
    ...
    mCurrentFrame.mNameFile = filename;
    mCurrentFrame.mnDataset = mnNumDataset;
    mCurrentFrame.mpfeats_down_body = pfeats_down_body;
    mCurrentFrame.mbprior_pose = false;
    mCurrentFrame.stereo_setflag = false;
    int framepclsize;
    framepclsize = mCurrentFrame.mpfeats_down_body->points.size();
    std::cout << "framepclsize0: " << framepclsize << std::endl;
...
bool Tracking::getIniertialBA2()
{
    return mbInertialBA2;
}

// void Tracking::SetIKdtree(KD_TREE<pcl::PointXYZINormal>* pIKdtree)
// {
//     mpIKdtree = pIKdtree;
//     int n = mpIKdtree->size();
//     std::cout << "IKdtree size in Tracker: " << n << std::endl;

// }

void Tracking::SetTleftb(const Sophus::SE3f& Tleftb)
{
    mTleftb = Tleftb;
    mTbleft = mTleftb.inverse();
    std::cout << "Tleftb in Tracker: " << mTleftb.matrix() << std::endl;
}
// void Tracking::CheckIKdtree_size()
// {
//     int n = mpIKdtree->size();
//     std::cout << "IKdtree size in Tracker: " << n << std::endl;
// }
...
void Tracking::PreintegrateIMU()
{
    ...
    {
        // std::cout << "No previous frame, the first frame" << std::endl;
    ...
    {
        // std::cout << "No IMU data in mlQueueImuData!!" << std::endl;
    ...
    if(!mCurrentFrame.mpImuPreintegrated){
        std::cout << "preintegration in current frame 1 does not exist" << std::endl;
    }
    mCurrentFrame.setIntegrated();
...
bool Tracking::PredictStateIMU()
{
    ...
    if(mbMapUpdated && mpLastKeyFrame)
    {
        ...
        std::cout << "IMU prediction!! keyframe" << std::endl;
        return true;
    }
    else if(!mbMapUpdated)
    {
        ...
        mCurrentFrame.mPredBias = mCurrentFrame.mImuBias;
        std::cout << "IMU prediction!! not keyframe" << std::endl;
        return true;
    }
    else
...
void Tracking::Track()
{
...
#endif
        // std::cout << "Preintegrating IMU data" << std::endl;
        PreintegrateIMU();
#ifdef REGISTER_TIMES
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if((!mbVelocity && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId<mnLastRelocFrameId+2)
                {
                    ...
                    // std::cout << "Tracking: 6.1" << std::endl;
                }
                else
                {
                ...
                // std::cout << "Tracking: 6.2" << std::endl;
                ...
                if (!bOK)
                {
                    ...
                    // std::cout << "Tracking: 6.3" << std::endl;
                }
            }
            else
            {
        ...
        else
        {
            ...
            // std::cout << "Tracking: 8" << std::endl;
        }
        // std::cout << "Tracking: 81" << std::endl;
        ...
        if(!mbOnlyTracking)
        {
            if(bOK)
            {
                ...
                if(false)
                {
                    ...
                    if(relo)
                    {
                        std::cout << "Relocalization after loop closure true" << std::endl;
                    }
                    else
                    {
                        std::cout << "Relocalization after loop closure false" << std::endl;
                    }
                    if(mnLoopBuffer <0)
                    {
                        mnLoopBuffer = 0;
        ...
        // std::cout << "Tracking: 9.1" << std::endl;
        if(bOK)
            mState = OK;
        else if (mState == OK)
        {
        ...
        // std::cout << "Tracking: 9.2" << std::endl;
        ...
        // std::cout << "Tracking: 9.3" << std::endl;
        if(pCurrentMap->isImuInitialized())
        {
            if(bOK)
            {
...
#endif
        // std::cout << "Tracking: 101" << std::endl;
        // Update drawer
        mpFrameDrawer->Update(this);
        if(mCurrentFrame.isSet())
            mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.GetPose());
        // std::cout << "Tracking: 102" << std::endl;
    ...
    if(mState==OK || mState==RECENTLY_LOST)
    {
        ...
        // std::cout << "Tracking: 13" << std::endl;
    }
    // std::cout << "Tracking: 145" << std::endl;
#ifdef REGISTER_LOOP
    if (Stop()) {
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
        {
            ...
            {
                if(!mCurrentFrame.mpImuPreintegrated){
                    std::cout << "No preintegration in current frame" << std::endl;
                }
                if(!mLastFrame.mpImuPreintegrated){
                    std::cout << "No preintegration in last frame" << std::endl;
                }
...
void Tracking::CreateMapInAtlas()
{
    ...
    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && mpImuPreintegratedFromLastKF)
    {
        ...
        mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(),*mpImuCalib);
        if(mpImuPreintegratedFromLastKF){
            std::cout << "IMU preintegration from last KF deleted and created in mapatlas" << std::endl;
        }
    }
...
{
    std::cout << "TRACK_REF_KF: T1" << std::endl;
    ...
    std::cout << "TRACK_REF_KF: T2" << std::endl;
    if(nmatches<15)
    {
    ...
    std::cout << "TRACK_REF_KF: T3" << std::endl;
    std::cout << "TRACK_REF_KF: T32" << std::endl;
    std::cout << "TRACK_REF_KF: T33" << std::endl;
    // cout << " TrackReferenceKeyFrame mLastFrame.mTcw:  " << mLastFrame.mTcw << endl;
    Optimizer::PoseOptimization(&mCurrentFrame);
    std::cout << "TRACK_REF_KF: T31" << std::endl;
    // Discard outliers
    int nmatchesMap = 0;
    for(int i =0; i<mCurrentFrame.N; i++)
    {
        ...
        {
            std::cout << "TRACK_REF_KF: T32n" << std::endl;
    ...
    std::cout << "TRACK_REF_KF: T4" << std::endl;
    if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
        return true;
    else
...
bool Tracking::TrackWithMotionModel()
{
    ...
    else
    {
        ...
        std::cout << "TrackWithMotionModel: T1" << std::endl;
    }
    ...
    Optimizer::PoseOptimization(&mCurrentFrame);
    std::cout << "TrackWithMotionModel: T2" << std::endl;
    // Discard outliers
    int nmatchesMap = 0;
    for(int i =0; i<mCurrentFrame.N; i++)
    {
...
bool Tracking::TrackLocalMap()
{
    ...
    int inliers;
    if (!mpAtlas->isImuInitialized()){
        if(mCurrentFrame.mbprior_pose)
        {
            // create a eigen 3d identity matrix
            Eigen::Matrix3d Ryc0 = Eigen::Matrix3d::Identity();
            Eigen::Vector3d tyc0 = Eigen::Vector3d::Zero();
            // Eigen::JacobiSVD<Eigen::Matrix3d> svd_wg(Ryc0, Eigen::ComputeFullU | Eigen::ComputeFullV);
            // Eigen::Matrix3d Ryc0_orthogonal = svd_wg.matrixU() * svd_wg.matrixV().transpose();
            Sophus::SE3d Tyc0(Ryc0, tyc0);
            Sophus::SE3d Tc0y = Tyc0.inverse();
            Sophus::SE3d Tybn = Tyc0 * mCurrentFrame.mTc0bn;
            Eigen::Vector3d velocity_y_bn = Tyc0.so3()*mCurrentFrame.mvelocity_c0.cast<double>();
            Eigen::Matrix3d mRwb_p = Tybn.so3().matrix();
            Eigen::JacobiSVD<Eigen::Matrix3d> svd(mRwb_p, Eigen::ComputeFullU | Eigen::ComputeFullV);
            Eigen::Matrix3d Rwb_orthogonal = svd.matrixU() * svd.matrixV().transpose();
            mCurrentFrame.mRwb_p = Rwb_orthogonal;
            // mCurrentFrame.mRwb_p = Tybn.so3().matrix();
            mCurrentFrame.mtwb_p = Tybn.translation();
            mCurrentFrame.stereo_setflag = true;
        }
        else
        {
            mCurrentFrame.stereo_setflag = false;
        ...
        Optimizer::PoseOptimization(&mCurrentFrame);
        std::cout << "TrackLocalMap: Pure PO" << std::endl;
    }
    else
    {
        if(mCurrentFrame.mnId<=mnLastRelocFrameId+mnFramesToResetIMU)
        ...
        else
        {
            ...
            if(!mbMapUpdated) //  && (mnMatchesInliers>30))
            {
                ...
                int framepclsize;
                if(mCurrentFrame.mpfeats_down_body == nullptr)
                {
                    // std::cout << "this frame has a empty pcl ptr" << std::endl;
                    // update the prior pose of the frame
                    Eigen::Matrix3d Ryc0 = mRowg.cast<double>();
                    Eigen::Vector3d tyc0 = Eigen::Vector3d::Zero();
                    Eigen::JacobiSVD<Eigen::Matrix3d> svd_wg(Ryc0, Eigen::ComputeFullU | Eigen::ComputeFullV);
                    Eigen::Matrix3d Ryc0_orthogonal = svd_wg.matrixU() * svd_wg.matrixV().transpose();
                    Sophus::SE3d Tyc0(Ryc0_orthogonal, tyc0);
                    Sophus::SE3d Tc0y = Tyc0.inverse();
                    Sophus::SE3d Tybn = Tyc0 * mCurrentFrame.mTc0bn;
                    Eigen::Vector3d velocity_y_bn = Tyc0.so3()*mCurrentFrame.mvelocity_c0.cast<double>();
                    Eigen::Matrix3d mRwb_p = Tybn.so3().matrix();
                    ...
                    // mCurrentFrame.mRwb_p = Tybn.so3().matrix();
                    mCurrentFrame.mtwb_p = Tybn.translation();
                    mCurrentFrame.mvelocity_y_bn = velocity_y_bn;
                    mCurrentFrame.mTTwb_p = Sophus::SE3d(Tybn.so3().matrix(), Tybn.translation());
                    // std::cout << "mCurrentFrame.mTwb_p:" << mCurrentFrame.mTTwb_p.matrix() << std::endl;
                ...
                {
                    framepclsize = mCurrentFrame.mpfeats_down_body->points.size();
                    std::cout << "framepclsize before PO: " << framepclsize << std::endl;
                    // inliers = Optimizer::PoseInertialOptimizationLastFrame_PCL(&mCurrentFrame, mpIKdtree, mCurrentFrame.mpfeats_down_body, mRowg.cast<double>(), false);
                }
    ...
    // copy the mvl_WorldCoords, mvr_WorldCoords, mvl_observations, mvr_observations from frame to tracking
    // mvl_WorldCoords = mCurrentFrame.mvl_WorldCoords;
    // mvr_WorldCoords = mCurrentFrame.mvr_WorldCoords;
    // mvl_observations = mCurrentFrame.mvl_observations;
    // mvr_observations = mCurrentFrame.mvr_observations;
    ...
    {
        std::cout << "TrackLocalMap: Not enough matches after PO" << std::endl;
    ...
    else if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
    {
        if(mnMatchesInliers<15)
        {
            if(mCurrentFrame.mbprior_pose)
            {
                std::cout << "TrackLocalMap: with prior" << std::endl;
                return true;
            }
            std::cout << "TrackLocalMap: mnMatchesInliers<15" << std::endl;
            return false;
        }
        else
            return true;
    ...
    else
    {
        ...
        {
            std::cout << "TrackLocalMap: mnMatchesInliers<30" << std::endl;
            return false;
        }
        else
        {
...
void Tracking::CreateNewKeyFrame()
{
    ...
    std::cout << "KF created with bool mbprior_pose;: " << pKF->mbprior_pose << std::endl;
    ...
    if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
    {
        ...
        if(mpImuPreintegratedFromLastKF){
            std::cout << "IMU preintegration from last KF created in KF creation" << std::endl;
        }
    }
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(bTcw)
            {
                ...
                if(nGood<50)
                {
                    std::cout<<"nGood<50, debug"<<std::endl;
                    int nadditional =matcher2.SearchByProjection(mCurrentFrame,vpCandidateKFs[i],sFound,10,100);
                    std::cout<<"debug1 ok"<<std::endl;
