...
public:
    ...
    // int static PoseInertialOptimizationLastKeyFrame_PCL(Frame *pFrame, KD_TREE<pcl::PointXYZINormal>* pIKdtree, pcl::PointCloud<pcl::PointXYZINormal>::Ptr pfeats_down_body, Eigen::Matrix3d Ryc0, bool bRecInit = false);

    int static PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit = false);

    // int static PoseInertialOptimizationLastFrame_PCL(Frame *pFrame, KD_TREE<pcl::PointXYZINormal>* pIKdtree, pcl::PointCloud<pcl::PointXYZINormal>::Ptr pfeats_down_body, Eigen::Matrix3d Ryc0, bool bRecInit = false);

    bool mpoint_selected_surf[100000];

    bool static esti_plane(Eigen::Matrix<float, 4, 1> &pca_result, const std::vector<pcl::PointXYZINormal, Eigen::aligned_allocator<pcl::PointXYZINormal>> &point, const float &threshold); 
