...
class EdgePriorPoseLIO : public g2o::BaseUnaryEdge< 6, Vector6d, VertexPose>
{
public:
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW
        EdgePriorPoseLIO(const Eigen::Matrix3d &Rwb_, const Eigen::Vector3d &twb_):Rwb(Rwb_), twb(twb_){}
        virtual bool read(std::istream& is){return false;}
        virtual bool write(std::ostream& os) const{return false;}
...
public: const Eigen::Matrix3d Rwb;
...
public:
    ...
    void computeError(){
        ...
        // std::cout << "Error for inertial: " << _error << std::endl;
    }
...
class EdgeLiDARPlane : public g2o::BaseUnaryEdge<1, float, VertexPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeLiDARPlane( const Eigen::Vector3d &p_body_, const Eigen::Vector3d &p_lidar_, const Eigen::Matrix3d &Rc0y_): p_body(p_body_), p_lidar(p_lidar_), Rc0y(Rc0y_){}
    virtual void computeError() override {
        const VertexPose* VPose = static_cast<const VertexPose*>(_vertices[0]);
        Eigen::Vector3d p_global_y;
        p_global_y = VPose->estimate().Rwb*p_body + VPose->estimate().twb;
        Eigen::Vector3d p_global_c0;
        p_global_c0 = Rc0y*p_global_y;
        Eigen::Vector3d p_global_i0;
        p_global_i0 = VPose->estimate().Rbc[0] * p_global_c0 + VPose->estimate().tbc[0];
        _error[0] = pabcd(0) * p_global_i0(0) + pabcd(1) * p_global_i0(1) + pabcd(2) * p_global_i0(2) + pabcd(3);
    }

    // Placeholder methods for reading and writing the edge, if needed
    ...
    virtual void linearizeOplus() override{
        const VertexPose* VPose = static_cast<const VertexPose*>(_vertices[0]);
        Eigen::Matrix3d p_lidar_crossmat;
        p_lidar_crossmat << SKEW_SYM_MATRX(p_lidar);
        Eigen::Matrix3d p_body_crossmat;
        p_body_crossmat << SKEW_SYM_MATRX(p_body);
        Eigen::Matrix3d Rybn;
        Rybn = VPose->estimate().Rwb;
        Rybn = -1.0 * Rybn;
        Eigen::Matrix3d Rby;
        Rby = VPose->estimate().Rbc[0]*Rc0y;
        Eigen::Matrix3d A;
        A = Rybn*p_body_crossmat;
        Eigen::Matrix3d B;
        B = Rby*A;
        Eigen::Matrix<double, 1, 3> C;
        C << normal(0), normal(1), normal(2);
        Eigen::Matrix<double, 1, 3> C_;
        C_ = C*B;
        // convert C_ to Vector3d
        Eigen::Vector3d C__;
        C__ << C_(0), C_(1), C_(2);
        //_jacobianOplusXi is 1x6, the first 1x3 is C*B
        _jacobianOplusXi.block<1,3>(0,0) = C__.transpose();
        Eigen::Vector3d D;
        D = Rby*normal;
        _jacobianOplusXi.block<1,3>(0,3) = D.transpose();
    }
